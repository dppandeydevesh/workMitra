...
export default function Preferences() {const navigate = useNavigate();
 ...
 const handleAddCustomSkill = () => {const trimmed = customSkill.trim();
 if (trimmed && !preferences.skills.includes(trimmed)) {setPreferences({...preferences, skills: [...preferences.skills, trimmed]});
}
 setCustomSkill("");
};

 const handleAddCustomInterest = () => {const trimmed = customInterest.trim();
 if (trimmed && !preferences.interests.includes(trimmed)) {setPreferences({...preferences, interests: [...preferences.interests, trimmed]});
}
 setCustomInterest("");
};

 const handleSkillToggle = (skill) => {if (preferences.skills.includes(skill)) {setPreferences({...preferences, skills: preferences.skills.filter(s => s !== skill)});
} else {setPreferences({...preferences, skills: [...preferences.skills, skill]});
}
};

 const handleInterestToggle = (interest) => {if (preferences.interests.includes(interest)) {setPreferences({...preferences, interests: preferences.interests.filter(i => i !== interest)});
} else {setPreferences({...preferences, interests: [...preferences.interests, interest]});
}
};
 ...
 const savedUser = localStorage.getItem("user");
 if (!savedUser) {setErrorMessage(t("preferences.auth_missing_error"));
 return;
}
 ...
 body: JSON.stringify({fullName: preferences.name || parsedUser.fullName,
 ...
 if (!profileResponse.ok) {const errData = await profileResponse.json();
 ...
 if (response.ok) {// Update local memory cache states cleanly
 ...
 <h1 className="text-3xl font-bold text-ink-800">{t("preferences.title")}</h1>
 ...
 {/* Name */}
 <div className="mb-6">
 <label className="block text-sm font-medium text-ink-700 mb-1">{t("preferences.name_label")}</label>
 <input
 type="text"value={preferences.name}
 onChange={(e) => setPreferences({...preferences, name: e.target.value})}
 placeholder={t("preferences.name_placeholder")}
 className="w-full px-4 py-2 border border-ink-300 bg-transparent rounded-lg focus:ring-2 focus:ring-marigold-500 outline-none"required
 />
 </div>
 ...
 <div>
 <label className="block text-xs font-semibold text-ink-600 mb-1">{t("preferences.github_label")}</label>
 <input
 type="url"value={preferences.githubUrl}
 onChange={(e) => setPreferences({...preferences, githubUrl: e.target.value})}
 placeholder="https://github.com/username"className="w-full px-3 py-2 text-xs border border-ink-300 bg-transparent rounded-lg focus:ring-2 focus:ring-marigold-500 outline-none"/>
 </div>
 <div>
 <label className="block text-xs font-semibold text-ink-600 mb-1">{t("preferences.linkedin_label")}</label>
 <input
 type="url"value={preferences.linkedinUrl}
 onChange={(e) => setPreferences({...preferences, linkedinUrl: e.target.value})}
 placeholder="https://linkedin.com/in/amit-kumar"className="w-full px-3 py-2 text-xs border border-ink-300 bg-transparent rounded-lg focus:ring-2 focus:ring-marigold-500 outline-none"/>
 </div>
 <div>
 <label className="block text-xs font-semibold text-ink-600 mb-1">{t("preferences.portfolio_label")}</label>
 <input
 type="url"value={preferences.portfolioUrl}
 onChange={(e) => setPreferences({...preferences, portfolioUrl: e.target.value})}
 placeholder="https://username.dev"className="w-full px-3 py-2 text-xs border border-ink-300 bg-transparent rounded-lg focus:ring-2 focus:ring-marigold-500 outline-none"/>
 </div>
 <div>
 <label className="block text-xs font-semibold text-ink-600 mb-1">{t("preferences.avatar_label")}</label>
 <input
 type="url"value={preferences.avatarUrl}
 onChange={(e) => setPreferences({...preferences, avatarUrl: e.target.value})}
 placeholder="https://workmitra.me/assets/default-avatar.jpg"className="w-full px-3 py-2 text-xs border border-ink-300 bg-transparent rounded-lg focus:ring-2 focus:ring-marigold-500 outline-none"/>
 </div>
 </div>
 </div>
 ...
 {/* Bio / About */}
 <div className="mb-6">
 <label className="block text-sm font-medium text-ink-700 mb-1">{t("preferences.bio_label")}</label>
 <textarea
 value={preferences.bio}
 onChange={(e) => setPreferences({...preferences, bio: e.target.value})}
 placeholder={t("preferences.bio_placeholder")}
 rows="3"className="w-full px-4 py-2 border border-ink-300 bg-transparent rounded-lg focus:ring-2 focus:ring-marigold-500 outline-none"/>
 </div>

 {/* Experience Level */}
 <div className="mb-6">
 <label className="block text-sm font-medium text-ink-700 mb-2">{t("preferences.experience_label")}</label>
 ...
 <span className="capitalize text-sm text-ink-700 font-medium">{t(`preferences.experience_${level}`)}</span>
 </label>
 ))}
 </div>
 </div>
 ...
 {/* Skills */}
 <div className="mb-6">
 <label className="block text-sm font-medium text-ink-700 mb-2">{t("preferences.skills_label")}</label>
 <div className="flex flex-wrap gap-2">
 {skillsList.map(skill => (
 <button
 key={skill}
 type="button"onClick={() => handleSkillToggle(skill)}
 className={`px-3 py-1 rounded-full text-sm transition font-medium ${preferences.skills.includes(skill)
 ?"bg-marigold-500 text-white shadow-sm":"bg-ink-200 text-ink-700 hover:bg-ink-300"
}`}
 >
 {skill}
 </button>
 ...
 <button
 type="button"onClick={handleAddCustomSkill}
 ...
 {/* Interests */}
 <div className="mb-8">
 <label className="block text-sm font-medium text-ink-700 mb-2">{t("preferences.interests_label")}</label>
 <div className="flex flex-wrap gap-2">
 {interestsList.map(interest => (
 <button
 key={interest}
 type="button"onClick={() => handleInterestToggle(interest)}
 className={`px-3 py-1 rounded-full text-sm transition font-medium ${preferences.interests.includes(interest)
 ?"bg-green-600 text-white shadow-sm":"bg-ink-200 text-ink-700 hover:bg-ink-300"
}`}
 >
 {interest}
 </button>
 ...
 <button
 type="button"onClick={handleAddCustomInterest}
 ...
 {/* Submit Button */}
 <button
 type="submit"className="w-full bg-marigold-500 text-white py-3 rounded-lg hover:bg-marigold-600 transition font-medium shadow-md active:scale-[0.99]">
