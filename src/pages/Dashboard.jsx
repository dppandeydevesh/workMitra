...
export default function Dashboard() {
  ...
  const notificationsList = myApplications.map(app => {
    ...
    if (app.status === "Approved") {
      return { id: app._id, title: t("dashboard.appApprovedTitle") + " 🎉", message: t("dashboard.appApprovedMsg", { title: project.title }), date: app.appliedAt, type: "info" };
    }
    if (app.status === "Completed") {
      return { id: app._id, title: t("dashboard.gigCompletedTitle") + " 🏆", message: t("dashboard.gigCompletedMsg", { title: project.title, feedback: app.feedbackText || t("dashboard.excellentWork") }), date: app.submittedAt || app.appliedAt, type: "success" };
    }
    if (app.status === "Rejected") {
      return { id: app._id, title: t("dashboard.appRejectedTitle") + " ✕", message: t("dashboard.appRejectedMsg", { title: project.title }), date: app.appliedAt, type: "danger" };
    }
    return null;
  }).filter(Boolean);
              ...
              <div>
                ...
                {localStorage.getItem("hasPaidPass") === "true" ? (
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    ...
                    <button
                      onClick={() => { setCheckoutStep(1); setShowCheckoutModal(true); }}
              ...
              <div className="lg:col-span-1 bg-white border border-ink-200 p-4 sm:p-6 rounded-xl shadow-sm flex flex-col justify-between">
                <div>
                  ...
                  <form onSubmit={handleUpdateResumeDetails} className="space-y-4">
                    ...
                    <button
                      type="button" onClick={handleReviewCV}
                      ...
                      {(() => {
                        ...
                        return (
                          <div className="space-y-2">
                            <p className="text-[11px] text-[#3D4A5C] leading-normal">{t("dashboard.missingSkills")}</p>
          ...
          <div id="projects-section" className="mt-12 pt-8 border-t border-ink-100">
            ...
            <div className="bg-ink-50 border border-ink-100 p-4 rounded-xl mb-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
                ...
                {showSearchSuggestions && searchTerm && (
                  <div className="absolute left-0 right-0 mt-1 bg-white border border-ink-200 rounded-xl shadow-lg z-50 max-h-48 overflow-y-auto text-left">
                    {(() => {
                      ...
                      const suggestions = [];
                      projects.forEach(p => {
                        if (p.title && p.title.toLowerCase().includes(query) && !suggestions.includes(p.title)) { suggestions.push(p.title); }
              ...
              <div>
                ...
                <select value={skillFilter} onChange={(e) => setSkillFilter(e.target.value)} className="w-full bg-white border border-ink-200 text-xs px-3.5 py-2.5 rounded-xl focus:outline-none focus:ring-2 focus:ring-marigold-400">
              ...
              <div>
                ...
                <select value={workTypeFilter} onChange={(e) => setWorkTypeFilter(e.target.value)} className="w-full bg-white border border-ink-200 text-xs px-3.5 py-2.5 rounded-xl focus:outline-none focus:ring-2 focus:ring-marigold-400">
              ...
              <div>
                ...
                <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="w-full bg-white border border-ink-200 text-xs px-3.5 py-2.5 rounded-xl focus:outline-none focus:ring-2 focus:ring-marigold-400">
                  ...
                  return (
                    <motion.div key={project._id} onClick={() => navigate(`/project/${project._id}`)} className="bg-white rounded-xl border border-ink-200 p-4 sm:p-6 shadow-sm flex flex-col justify-between hover:border-marigold-400 hover:shadow-md transition-all duration-300 cursor-pointer wm-panel wm-card" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.15 }}>
                      <div>
                        ...
                        <h4 className="text-base font-bold text-ink-900 mb-2">{project.title}</h4>
                        <p className="text-xs text-ink-600 leading-relaxed mb-4 line-clamp-3">{project.description}</p>
                      </div>
                      ...
                      <div className="border-t pt-4 mt-2 space-y-3">
                        ...
                        <div className="flex justify-between items-center text-xs pt-1">
                          <div>
                            ...
                            <p className="font-bold text-green-600 text-sm">₹{project.budget?.toLocaleString()}</p>
                          </div>
