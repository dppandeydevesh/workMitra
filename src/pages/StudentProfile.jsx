  ...
  return (
    <div className="min-h-screen bg-transparent font-sans">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {loading ? (
          <div className="text-center py-16 text-ink-500 font-medium bg-white dark:bg-ink-900 rounded-2xl shadow-xl flex flex-col items-center justify-center space-y-3">
          ...
          <div className="bg-white dark:bg-ink-900 rounded-2xl shadow-xl p-8 text-center space-y-4">
            <div className="text-red-500 text-3xl">⚠️</div>
            <p className="text-red-700 font-bold">{errorMessage}</p>
            <button 
              onClick={() => navigate(-1)} 
              className="px-4 py-2 bg-ink-100 dark:bg-ink-800 hover:bg-ink-200 text-ink-700 dark:text-ink-200 rounded-xl text-xs font-bold transition"
            >
          ...
          <div className="space-y-6">
            {/* Header Card */}
            <div className="bg-white dark:bg-ink-900 rounded-2xl shadow-xl p-6 md:p-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 border border-white/60 dark:border-ink-800/60">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <div className="flex items-center flex-wrap gap-2">
                    <h1 className="text-2xl font-black text-ink-800 dark:text-ink-200 tracking-tight">{profileUser.fullName}</h1>
                    ...
                    {profileUser.githubUrl && (
                      <a href={profileUser.githubUrl} target="_blank" rel="noreferrer" className="text-ink-600 dark:text-ink-300 hover:text-black transition ml-1" title="GitHub">
                  ...
                  <p className="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-trustTeal-100 text-trustTeal-800 dark:bg-trustTeal-900/40 dark:text-trustTeal-300 mt-2 uppercase tracking-wider border border-trustTeal-200 dark:border-trustTeal-800/50">
                    ✓ {t("studentProfile.verifiedStudentProfile")}
                  </p>
                </div>
              </div>
            ...
            {/* Main Details Grid */}
            {isEditing ? (
              <form onSubmit={handleSaveProfile} className="bg-white dark:bg-ink-900 rounded-2xl shadow-xl p-6 md:p-8 space-y-6 border border-white/60 dark:border-ink-800/60">
                <h3 className="text-lg font-bold text-ink-800 dark:text-ink-200 border-b pb-3">{t("studentProfile.editPortfolioDetails")}</h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="flex items-center gap-3 pt-6">
                    ...
                    />
                    <label htmlFor="isProfilePrivate" className="text-xs font-bold text-ink-700 dark:text-ink-300 cursor-pointer">
                        ...
                        <button
                          ...
                          className={`px-3.5 py-2 rounded-xl text-xs font-bold transition border ${
                            isSelected 
                              ...
                              : "bg-ink-50 dark:bg-ink-900 hover:bg-ink-100 dark:bg-ink-800 text-ink-500 border-ink-200 dark:border-ink-800"
                          }`}
                        >
                          {slot}
                        </button>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-1 space-y-6">
                  <div className="bg-white dark:bg-ink-900 rounded-2xl shadow-xl p-6 border border-white/60 dark:border-ink-800/60 space-y-5">
                    <h3 className="text-sm font-black text-ink-800 dark:text-ink-200 uppercase tracking-wider border-b pb-2.5">{t("studentProfile.studentMetadata")}</h3>
                    
                    {/* Email */}
                    <div className="text-xs">
                      <span className="text-[10px] uppercase font-bold text-ink-400 block mb-0.5">{t("studentProfile.primaryEmail")}</span>
                      <span className="font-bold text-ink-700 dark:text-ink-200">{profileUser.email}</span>
                    </div>

                    {/* Mobile */}
                    <div className="text-xs">
                      <span className="text-[10px] uppercase font-bold text-ink-400 block mb-0.5">{t("studentProfile.mobileContact")}</span>
                      <span className="font-bold text-ink-700 dark:text-ink-200">{profileUser.mobile || t("studentProfile.notProvided")}</span>
                    </div>

                    {/* University */}
                    <div className="text-xs">
                      <span className="text-[10px] uppercase font-bold text-ink-400 block mb-0.5">{t("studentProfile.universityNode")}</span>
                      <span className="font-bold text-ink-700 dark:text-ink-200">{profileUser.collegeName || t("studentProfile.notVerified")}</span>
                    ...
                    <div className="text-xs">
                      ...
                      <span className="font-bold text-ink-700 dark:text-ink-200">{profileUser.projectType || t("studentProfile.remoteTrack")}</span>
                    </div>
                    ...
                    <div className="text-xs border-t pt-3">
                      ...
                      {avgRating ? (
                        <div className="flex items-center gap-1 mt-0.5">
                          ...
                          <span className="font-bold text-ink-800 dark:text-ink-200 text-sm">{avgRating} / 5</span>
                    ...
                    <div className="text-xs border-t pt-3 space-y-2">
                      ...
                      <div className="flex flex-col gap-1.5">
                        {profileUser.githubUrl ? (
                          <a href={profileUser.githubUrl} target="_blank" rel="noreferrer" className="text-ink-700 dark:text-ink-200 hover:text-purple-600 font-semibold flex items-center gap-1.5">
                    ...
                    {(profileUser.major || profileUser.currentSemester) && (
                      <div className="text-xs border-t pt-3">
                        ...
                        {profileUser.major && <p className="font-bold text-ink-700 dark:text-ink-300">{profileUser.major}</p>}
                ...
                <div className="lg:col-span-2 space-y-6">
                  {/* Bio & Skills Panel */}
                  <div className="bg-white dark:bg-ink-900 rounded-2xl shadow-xl p-6 md:p-8 border border-white/60 dark:border-ink-800/60 space-y-6">
                    <div>
                      <h3 className="text-sm font-black text-ink-800 dark:text-ink-200 uppercase tracking-wider border-b pb-2.5 mb-4">{t("studentProfile.professionalBio")}</h3>
                    ...
                    <div>
                      <h3 className="text-sm font-black text-ink-800 dark:text-ink-200 uppercase tracking-wider border-b pb-2.5 mb-4">{t("studentProfile.targetSkills")}</h3>
                      ...
                      <div className="border-t pt-4">
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {profileUser.preferredTechStack.map((tech, idx) => (
                            <span key={idx} className="bg-purple-50 text-purple-700 text-xs font-bold px-3 py-1 rounded-xl border border-purple-100 dark:border-ink-800">
                              #{tech}
                            </span>
                          ))}
                        </div>
                    ...
                    {profileUser.extracurriculars && profileUser.extracurriculars.length > 0 && (
                      <div className="border-t pt-4">
                        ...
                        <ul className="list-disc list-inside text-xs text-ink-600 dark:text-ink-300 space-y-1">
                  ...
                  {profileUser.videoPitchUrl && (
                    <div className="bg-white dark:bg-ink-900 rounded-2xl shadow-xl p-6 md:p-8 border border-white/60 dark:border-ink-800/60 space-y-4 print:hidden">
                      <h3 className="text-sm font-black text-ink-800 dark:text-ink-200 uppercase tracking-wider border-b pb-2.5">
                        🎥 {t("studentProfile.elevatorPitchTitle")}
                      </h3>
                      <div className="aspect-video w-full rounded-2xl overflow-hidden bg-ink-900 border">
                        <iframe
                  ...
                  {/* 📊 Skill Gap Analyzer */}
                  <div className="bg-white dark:bg-ink-900 rounded-2xl shadow-xl p-6 md:p-8 border border-white/60 dark:border-ink-800/60 space-y-4">
                    <h3 className="text-sm font-black text-ink-800 dark:text-ink-200 uppercase tracking-wider border-b pb-2.5">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="text-xs space-y-2 pt-2 text-ink-600 dark:text-ink-300">
                  ...
                  <div className="bg-white dark:bg-ink-900 rounded-2xl shadow-xl p-6 md:p-8 border border-white/60 dark:border-ink-800/60 space-y-4">
                    <div className="flex justify-between items-center border-b pb-2.5">
                      <h3 className="text-sm font-black text-ink-800 dark:text-ink-200 uppercase tracking-wider">
                    ...
                    {!profileUser.softSkillsVouches || profileUser.softSkillsVouches.length === 0 ? (
                      ...
                      <div className="space-y-3.5 max-h-60 overflow-y-auto pr-1">
                        {profileUser.softSkillsVouches.map((vouch, idx) => (
                          <div key={idx} className="bg-ink-50 dark:bg-ink-800/50 border p-3 rounded-xl space-y-1.5 text-left">
                            ...
                            <p className="text-xs text-ink-600 dark:text-ink-300 leading-relaxed font-semibold italic">
                              "{vouch.comment}"
                            </p>
                          </div>
                        ))}
                  ...
                  {/* AI Critique Panel */}
                  <div className="bg-gradient-to-br from-marigold-950 to-ink-900 text-white rounded-2xl shadow-xl p-6 md:p-8 space-y-6">
                          ...
                          <div key={app.applicationId || app._id} className="pt-3 first:pt-0 space-y-1">
                            <div className="flex justify-between items-center text-[10px]">
                              ...
                              <div className="flex items-center gap-0.5">
                                ...
                                <span className="font-extrabold text-ink-700 dark:text-ink-200 ml-1">({app.rating}/5)</span>
                              </div>
                            </div>
                            {app.ratingReview && (
                              <p className="text-xs text-ink-700 dark:text-ink-200 leading-relaxed font-semibold mt-1">
                                "{app.ratingReview}"
                              </p>
                            )}
                            {app.feedbackText && (
      ...
      {showVouchModal && (
        <div className="fixed inset-0 bg-ink-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-ink-900 rounded-3xl max-w-md w-full shadow-2xl p-6 border text-left flex flex-col animate-fade-in select-none">
            <div className="flex justify-between items-center border-b pb-3 mb-4">
              <h3 className="font-black text-ink-800 dark:text-ink-200 text-sm uppercase tracking-wider">{t("studentProfile.endorseBatchmate")}</h3>
              <button onClick={() => setShowVouchModal(false)} className="text-ink-400 hover:text-ink-600 dark:text-ink-300 text-lg">×</button>
            </div>
            <form
              onSubmit={async (e) => {
                e.preventDefault();
            ...
            >
              <div>
                ...
                <input
                  ...
                  className="w-full bg-ink-50 dark:bg-ink-800 border text-xs px-3.5 py-2.5 rounded-xl focus:outline-none focus:ring-2 focus:ring-marigold-400"
                  required
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full bg-ink-50 dark:bg-ink-800 border text-xs px-3.5 py-2.5 rounded-xl focus:outline-none focus:ring-2 focus:ring-marigold-400 resize-none"
                  required
                />
              </div>
