  ...
  const fetchPartners = async () => {
    ...
    try {
      const res = await fetchWithAuth(
        `${API_BASE_URL}/api/chat/partners/${loggedInUser.email}`,
        ...
        if (recipientEmail && recipientEmail !== loggedInUser.email) {
          ...
          if (!partnerExists) {
            const userRes = await fetchWithAuth(
              `${API_BASE_URL}/api/auth/user/${recipientEmail}`,
              { credentials: 'include', headers: {} }
            );
            if (userRes.ok) {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      ...
      try {
        ...
        const partnerRes = await fetchWithAuth(
          `${API_BASE_URL}/api/chat/partners/${loggedInUser.email}`,
          { credentials: 'include', headers: {} }
        );
        if (partnerRes.ok) {
  ...
  useEffect(() => {
    ...
    const handleIncomingData = (data) => {
      ...
      if (data.type === 'message') {
        ...
        if (
          (data.sender === currentActivePartner?.email &&
            data.receiver === loggedInUser.email) ||
          (data.sender === loggedInUser.email &&
            data.receiver === currentActivePartner?.email)
        ) {
          setMessages((prev) => [...prev, data]);

          // Mark incoming active partner messages as read immediately
          if (data.sender === currentActivePartner?.email) {
            fetchWithAuth(`${API_BASE_URL}/api/chat/read`, {
              credentials: 'include',
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
  ...
  return (
    ...
    >
      ...
      <div className="max-w-6xl w-full mx-auto wm-panel flex flex-col md:flex-row flex-1 overflow-hidden min-h-[500px] sm:min-h-[600px] z-10">
        ...
        {/* ========================================================================= */}
        <div
          className={`w-full md:w-[340px] wm-panel border-r border-white/20 flex-col bg-white/30 ${showMobileChat ? 'hidden md:flex' : 'flex'}`}
        >
          <div className="p-5 sm:p-6 border-b border-white/20">
            <h2 className="text-xl sm:text-2xl font-extrabold bg-gradient-to-r from-marigold-600 to-marigold-600 bg-clip-text text-transparent">
              {t('chat.title')}
            </h2>
            ...
            {loadingPartners ? (
              <div className="space-y-3">
                {[1, 2, 3].map((i) => (
                  <div
                    key={i}
                    className="h-16 rounded-xl border border-white/20 skeleton-loader wm-panel"
                  ></div>
                ))}
              </div>
            ) : partnersError ? (
              <div className="p-4 text-center space-y-2.5">
                ...
                <button
                  ...
                  className="px-3.5 py-1.5 rounded-lg font-bold text-[11px] transition active:scale-95 shadow-sm"
                >
                  Retry
                </button>
              </div>
              ...
              <div className="wm-panel p-[24px_16px] text-center max-w-sm mx-auto my-4 flex flex-col items-center justify-center">
                ...
                <div>
                  <h3 className="text-[14px] font-semibold text-[#1B2333] mb-[4px]">
                    {t('chat.noConversations')}
                  </h3>
                  <p className="text-[12px] text-[#6B7280] leading-normal max-w-[200px] mx-auto">
                    No messages active. Match with projects or contact
                    recruiters to begin a conversation.
                  </p>
                </div>
              </div>
            ) : (
              partners.map((partner) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-4 p-3.5 rounded-xl transition-all duration-300 text-left ${
                      isActive
                        ? 'bg-white/60 shadow-sm border border-white/40 scale-[1.02]'
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <h4
                        className={`text-sm font-bold truncate ${isActive ? 'text-ink-900' : 'text-ink-700'}`}
                      >
        ...
        {/* ========================================================================= */}
        <div
          className={`flex-1 wm-panel flex-col bg-white/40 relative ${!showMobileChat ? 'hidden md:flex' : 'flex'}`}
        >
          {activePartner ? (
            <>
              ...
              <div className="p-4 sm:p-5 border-b border-white/20 flex items-center justify-between bg-white/50 z-10 sticky top-0">
                <div className="flex items-center gap-4">
                  <button
                    ...
                    className="md:hidden text-ink-600 hover:text-marigold-500 p-2 rounded-xl hover:bg-white/50 transition-colors"
                  ...
                  <div className="relative">
                    ...
                    <div
                      className={`absolute bottom-0 right-0 w-3.5 h-3.5 rounded-full border-2 border-white ${wsConnected ? 'bg-green-500' : 'bg-ink-400'}`}
                    ></div>
                  </div>
                  <div>
                        ...
                        {isPartnerTyping ? (
                          <span className="text-marigold-500 font-bold flex items-center gap-1">
                            ...
                            <span className="flex space-x-0.5 ml-1">
                              <span
                                className="w-1 h-1 bg-marigold-500 rounded-full animate-bounce"
                  ...
                  messages.map((msg, index) => {
                    ...
                    return (
                      ...
                      >
                        {!isOutgoing && (
                          <div
                            className={`w-6 h-6 rounded-full bg-[#F5A623] text-white font-bold flex items-center justify-center text-[10px] shadow-sm shrink-0 ${showAvatar ? 'opacity-100' : 'opacity-0'}`}
                          >
                            {showAvatar
                              ? activePartner.fullName
                                ? activePartner.fullName.charAt(0).toUpperCase()
                        ...
                        <div
                          className={`max-w-[75%] px-4 py-3 shadow-sm text-[13px] leading-relaxed relative group ${
                            isOutgoing
                              ...
                              : 'bg-white/90 text-ink-800 border border-ink-200 rounded-xl rounded-bl-sm'
                          }`}
                          style={
                            isOutgoing
                              ? { background: '#1B2333', color: '#FFFFFF' }
                              : {}
                          }
                        >
                          ...
                          <span
                            className={`text-[9px] block font-medium opacity-0 group-hover:opacity-100 transition-opacity absolute ${
                              isOutgoing
                                ? '-left-12 bottom-2 text-ink-500'
                                : '-right-12 bottom-2 text-ink-500'
                            }`}
                          >
                            {msg.timestamp
                              ? (() => {
              ...
              <div className="p-3 sm:p-4 border-t border-white/20 bg-white/50 z-10">
                ...
                >
                  ...
                  <input
                    ...
                    className="flex-1 bg-white/70 border border-white/50 text-sm px-5 py-3.5 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition-all shadow-inner text-ink-800 placeholder-ink-500"
                    required
                  />
                  <button
                    type="submit"
                    className="bg-gradient-to-r from-marigold-600 to-marigold-600 hover:from-marigold-700 hover:to-marigold-700 text-white font-bold text-sm px-5 sm:px-6 rounded-xl shadow-md transition-transform hover:scale-105 active:scale-95 flex items-center justify-center disabled:opacity-50 disabled:hover:scale-100"
