...
const AIAssistant = () => {
  ...
  const toggleChat = () => {
    ...
    if (!isOpen && messages.length === 0) {
      ...
      setMessages([
        { sender: 'ai', text: t('dashboard.aiAssistantGreeting', { name }) },
      ]);
    }
  };
      ...
      setMessages((prev) => [
        ...prev,
        {
          ...
          text:
            ...
            data.reply ||
            data.message ||
            t('Sorry, I could not process that.'),
        },
      ]);
    } catch (error) {
  ...
  // Students get a BottomNav bar on phones — lift the launcher above it
  let isStudent = false;
  try {
    isStudent =
      JSON.parse(localStorage.getItem('user') || 'null')?.userRole ===
      'student';
  } catch {
    isStudent = false;
  }
  ...
  return (
    <div
      className={`fixed ${isStudent ? 'bottom-24 md:bottom-6' : 'bottom-6'} right-6 z-50 flex flex-col items-end`}
    >
      {/* Chat Window Panel */}
      {isOpen && (
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-marigold-100 opacity-90">
                    ...
                    <div className="prose prose-sm prose-p:my-1 prose-ul:my-1 max-w-none break-words">
                      ...
                      {index === 0 && (
                        <div className="flex flex-wrap gap-2 mt-3 select-none">
                          {[
                            t('dashboard.chipFindGigs'),
                            t('dashboard.chipReviewProfile'),
                            t('dashboard.chipInterviewTips'),
                          ].map((chipText) => (
                            <button
                              key={chipText}
                              type="button"
      ...
      <button
        onClick={toggleChat}
        className={`w-14 h-14 rounded-full flex items-center justify-center shadow-sm transition-all duration-300 ${
          isOpen
