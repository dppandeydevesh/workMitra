...
export default function CompanyDashboard() {
  ...
  const getStatusColor = (status) => {
    const map = {
      ...
      Submitted: 'text-ink-dark bg-paper border-border',
      ...
      <div className="min-h-screen bg-transparent flex items-center justify-center px-4">
        <div className="max-w-md w-full bg-white rounded-xl shadow-sm p-8 border border-ink-100 text-center space-y-4">
          ...
          <h2 className="text-lg font-bold text-ink-800">
            {t('companyDashboard.toastFailedToLoadDashboard')}
          </h2>
  ...
  return (
    <div className="min-h-screen bg-transparent font-sans">
      ...
      >
        {/* Welcome Header */}
        <motion.div
          className="bg-gradient-to-r from-marigold-600 via-paper to-marigold-600 rounded-xl shadow-sm p-6 sm:p-8 mb-8 text-white relative overflow-hidden"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ duration: 0.15 }}
        >
          ...
          <div className="relative z-10">
            <p className="text-marigold-200 text-[10px] font-extrabold uppercase tracking-widest mb-1">
              {t('companyDashboard.commandCenter')}
            </p>
            <h1 className="text-2xl sm:text-3xl font-black tracking-tight">
              {t('companyDashboard.welcomeBack', {
                name:
                  currentUser?.companyName ||
                  currentUser?.fullName ||
                  t('companyDashboard.defaultRecruiterName'),
              })}{' '}
              👋
            </h1>
            ...
            <div className="flex flex-wrap gap-3 mt-5">
              <span className="wm-panel px-3 py-1.5 rounded-xl text-[10px] font-extrabold flex items-center gap-1.5 text-white border-white/20">
                ...
                {t('companyDashboard.statProjects', {
                  count: stats?.totalProjects || 0,
                })}
              </span>
              <span className="wm-panel px-3 py-1.5 rounded-xl text-[10px] font-extrabold flex items-center gap-1.5 text-white border-white/20">
                <Users className="w-3.5 h-3.5" />{' '}
                {t('companyDashboard.statApplications', {
                  count: stats?.totalApplications || 0,
                })}
              </span>
              <span className="wm-panel px-3 py-1.5 rounded-xl text-[10px] font-extrabold flex items-center gap-1.5 text-white border-white/20">
                <Star className="w-3.5 h-3.5 text-marigold-300 fill-current" />{' '}
                {t('companyDashboard.statAvgRating', {
        ...
        >
          {[
            {
              label: t('companyDashboard.kpiTotalProjects'),
              value: stats?.totalProjects || 0,
              icon: <Folder className="w-4 h-4" />,
              color: 'from-marigold-500 to-marigold-600',
            },
            {
              label: t('companyDashboard.kpiActiveApplications'),
              value: stats?.totalApplications || 0,
              icon: <Inbox className="w-4 h-4" />,
              color: 'from-paper to-paper',
            },
            {
              label: t('companyDashboard.kpiUnderReview'),
              value: stats?.submittedCount || 0,
              icon: <Search className="w-4 h-4" />,
              color: 'from-amber-500 to-amber-600',
            },
            {
              label: t('companyDashboard.kpiCompletedTasks'),
              value: stats?.completedCount || 0,
              icon: <CheckCircle className="w-4 h-4" />,
              color: 'from-green-500 to-green-600',
            },
            {
              label: t('companyDashboard.kpiTotalBudget'),
              value: `₹${(stats?.totalBudget || 0).toLocaleString()}`,
              icon: <DollarSign className="w-4 h-4" />,
              color: 'from-emerald-500 to-emerald-600',
            },
            {
              label: t('companyDashboard.kpiAvgRating'),
            ...
            <div key={idx} className="wm-panel wm-card p-4 group rounded-xl">
              <div
                className={`w-8 h-8 rounded-xl bg-gradient-to-br ${kpi.color} flex items-center justify-center text-white text-sm mb-3 group-hover:scale-110 transition-transform`}
              >
                {kpi.icon}
              </div>
              ...
              <div className="flex items-center gap-1 mt-0.5">
                ...
                {kpi.label === t('companyDashboard.kpiAvgRating') && (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 space-y-6">
            {/* Quick Actions */}
            <motion.div
              className="wm-panel p-5 rounded-xl"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              transition={{ duration: 0.15 }}
            >
              ...
              <div className="space-y-2.5">
                {[
                  {
                    label: t('companyDashboard.actionDeploy'),
                    icon: <Plus className="w-4 h-4" />,
                    path: '/add-project',
                    style:
                      'bg-gradient-to-r from-marigold-600 to-marigold-600 text-white shadow-md hover:shadow-lg',
                  },
                  {
                    label: t('companyDashboard.actionMyProjects'),
                    icon: <Folder className="w-4 h-4" />,
                    path: '/my-projects',
                    style: 'wm-panel wm-card text-marigold-700',
                  },
                  {
                    label: t('companyDashboard.actionApplicantsHub'),
                    icon: <Users className="w-4 h-4" />,
                    path: '/applicants',
                    style: 'wm-panel wm-card text-green-700',
                  },
                  {
                    label: t('companyDashboard.actionAnalytics'),
                    icon: <BarChart2 className="w-4 h-4" />,
                    path: '/analytics',
                    style: 'wm-panel wm-card text-ink-dark',
                  },
                  {
                    label: t('companyDashboard.actionCalendar'),
                    icon: <Calendar className="w-4 h-4" />,
                    path: '/calendar',
                    style: 'wm-panel wm-card text-amber-700',
                  },
                  {
                    label: t('companyDashboard.actionSettings'),
                    icon: <Settings className="w-4 h-4" />,
                    path: '/company-settings',
                    style: 'wm-panel wm-card text-ink-700',
                  },
                ].map((action, idx) => (
                  <button
                    key={idx}
            ...
            >
              <h3 className="text-[10px] font-black text-ink-400 uppercase tracking-wider mb-4 flex items-center gap-1.5">
                ...
                {t('companyDashboard.topPerformers')}
              </h3>
              {!stats?.topPerformers || stats.topPerformers.length === 0 ? (
                <div className="p-6 text-center flex flex-col items-center justify-center">
                  ...
                  <p className="text-[12px] font-medium text-[#1B2333] mb-[2px]">
                    {t('companyDashboard.noTopPerformers')}
                  </p>
                  <p className="text-[11px] text-[#6B7280] leading-normal max-w-[200px] mx-auto">
                    Complete gigs to rank high-performing students on your
                    roster.
                  </p>
                </div>
              ) : (
                <div className="space-y-3">
                  {stats.topPerformers.map((perf, idx) => (
                    <div
                      key={idx}
                      onClick={() =>
                        navigate(`/student-profile/${perf.studentEmail}`)
                      }
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[10px] text-ink-400 truncate flex items-center gap-1 mt-0.5">
                          <FileText className="w-3 h-3" /> {perf.projectTitle}
                        </p>
                      </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            <motion.div
              className="wm-panel p-5 sm:p-6 rounded-xl"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              transition={{ duration: 0.15 }}
            >
                ...
                <div className="wm-panel p-[32px_16px] text-center max-w-sm mx-auto my-4 flex flex-col items-center justify-center">
                  ...
                  <div>
                    <h4 className="text-[14px] font-semibold text-[#1B2333] mb-[4px]">
                      {t('companyDashboard.noRecentActivity')}
                    </h4>
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-0.5">
                          ...
                          <span
                            className={`text-[9px] font-extrabold px-1.5 py-0.5 rounded border uppercase ${getStatusColor(event.status)}`}
                          >
                            {event.status}
                          </span>
                        </div>
            ...
            {stats && (
              ...
              >
                <h3 className="text-[10px] font-black text-ink-400 uppercase tracking-wider mb-4 flex items-center gap-1.5">
                  ...
                  {t('companyDashboard.applicationPipeline')}
                </h3>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {[
                    {
                      label: t('companyDashboard.statusPending'),
                      count: stats.pendingCount,
                      color: 'bg-amber-500',
                    },
                    {
                      label: t('companyDashboard.statusApproved'),
                      count: stats.approvedCount,
                      color: 'bg-marigold-500',
                    },
                    {
                      label: t('companyDashboard.statusSubmitted'),
                      count: stats.submittedCount,
                      color: 'bg-paper',
                    },
                    {
                      label: t('companyDashboard.statusCompleted'),
                      count: stats.completedCount,
                      color: 'bg-green-500',
                    },
                    {
                      label: t('companyDashboard.statusRejected'),
                      count: stats.rejectedCount,
                      color: 'bg-red-500',
                    },
                    {
                      label: t('companyDashboard.statusInRevision'),
                    ...
                    <div
                      key={idx}
                      className="wm-panel wm-card rounded-xl p-3 text-center"
                    >
                      <div
