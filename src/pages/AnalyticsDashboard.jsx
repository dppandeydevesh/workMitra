...
export default function AnalyticsDashboard() {
  ...
  const totalBudgetInvested = projects.reduce(
    (sum, p) => sum + (p.budget || 0),
    0
  );
  const averageBudget =
    totalProjectsCount > 0
      ? Math.round(totalBudgetInvested / totalProjectsCount)
      : 0;
  ...
  const conversionPercentage =
    totalApplicationsCount > 0
      ? Math.round(
          ((statusCounts.Completed + statusCounts.Approved) /
            ...
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div className="bg-marigold-50 border border-marigold-100 p-6 rounded-xl shadow-sm hover:shadow-md transition">
                  ...
                  <span className="text-[10px] font-extrabold text-marigold-700 uppercase tracking-wider block">
                    {t('analytics.gigsDeployed')}
                  </span>
                  <span className="text-3xl font-black text-marigold-950 block mt-1">
                    {totalProjectsCount}
                  </span>
                  <p className="text-xs text-marigold-800/70 mt-1">
                    {t('analytics.gigsDeployedSub')}
                  </p>
                </div>
                ...
                <div className="bg-green-50 border border-green-100 p-6 rounded-xl shadow-sm hover:shadow-md transition">
                  ...
                  <span className="text-[10px] font-extrabold text-green-700 uppercase tracking-wider block">
                    {t('analytics.applicationsReceived')}
                  </span>
                  <span className="text-3xl font-black text-green-950 block mt-1">
                    {totalApplicationsCount}
                  </span>
                  <p className="text-xs text-green-800/70 mt-1">
                    {t('analytics.applicationsReceivedSub')}
                  </p>
                </div>

                {/* Metric 3 */}
                <div className="bg-paper border border-border p-6 rounded-xl shadow-sm hover:shadow-md transition">
                  <div className="text-2xl mb-3">🎯</div>
                  <span className="text-[10px] font-extrabold text-ink-dark uppercase tracking-wider block">
                    {t('analytics.conversionRate')}
                  </span>
                  <span className="text-3xl font-black text-ink-dark block mt-1">
                    {conversionPercentage}%
                  </span>
                  <p className="text-xs text-ink-dark mt-1">
                    {t('analytics.conversionRateSub')}
                  </p>
                </div>
                ...
                <div className="bg-amber-50 border border-amber-100 p-6 rounded-xl shadow-sm hover:shadow-md transition">
                  ...
                  <span className="text-[10px] font-extrabold text-amber-700 uppercase tracking-wider block">
                    {t('analytics.averageGigBudget')}
                  </span>
                  <span className="text-3xl font-black text-amber-950 block mt-1">
                    ₹{averageBudget.toLocaleString()}
                  </span>
                  <p className="text-xs text-amber-800/70 mt-1">
                    {t('analytics.totalBudget', {
                      total: totalBudgetInvested.toLocaleString(),
                    })}
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 bg-ink-50 p-6 rounded-xl border border-ink-100 shadow-inner">
                <div className="flex items-center gap-4">
                  <div className="text-3xl bg-marigold-100 p-3 rounded-xl text-marigold-700">
                    🔒
                  </div>
                  <div>
                    <span className="text-[10px] font-black text-marigold-700 uppercase tracking-wider block">
                      {t('analytics.lockedInEscrow')}
                    </span>
                    <span className="text-2xl font-black text-ink-800 block mt-0.5">
                      ₹{lockedEscrow.toLocaleString()}
                    </span>
                    <p className="text-[11px] text-ink-400 mt-0.5">
                      {t('analytics.lockedInEscrowSub')}
                    </p>
                  </div>
                </div>

                <div className="flex items-center gap-4">
                  <div className="text-3xl bg-emerald-100 p-3 rounded-xl text-emerald-700">
                    💸
                  </div>
                  <div>
                    <span className="text-[10px] font-black text-emerald-700 uppercase tracking-wider block">
                      {t('analytics.totalPayouts')}
                    </span>
                    <span className="text-2xl font-black text-ink-800 block mt-0.5">
                      ₹{releasedPayouts.toLocaleString()}
                    </span>
                    <p className="text-[11px] text-ink-400 mt-0.5">
                      {t('analytics.totalPayoutsSub')}
                    </p>
                  </div>
                </div>

                <div className="flex items-center gap-4">
                  <div className="text-3xl bg-rose-100 p-3 rounded-xl text-rose-700">
                    ⚠️
                  </div>
                  <div>
                    <span className="text-[10px] font-black text-rose-700 uppercase tracking-wider block">
                      {t('analytics.escrowDisputed')}
                    </span>
                    <span className="text-2xl font-black text-ink-800 block mt-0.5">
                      ₹{disputedEscrow.toLocaleString()}
                    </span>
                    <p className="text-[11px] text-ink-400 mt-0.5">
                      {t('analytics.escrowDisputedSub')}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="bg-white border border-ink-100 p-6 rounded-xl shadow-sm flex flex-col justify-between">
                  <div>
                    <h3 className="text-base font-bold text-ink-800 mb-1">
                      {t('analytics.applicationLifecycle')}
                    </h3>
                    <p className="text-xs text-ink-400 mb-6">
                      {t('analytics.applicationLifecycleSub')}
                    </p>

                    {totalApplicationsCount === 0 ? (
                      <p className="text-xs text-ink-400 italic text-center py-16">
                        {t('analytics.noApplications')}
                      </p>
                    ) : (
                      <div className="flex flex-col items-center">
                        ...
                        <div className="relative w-full h-48 flex items-center justify-center mb-6">
                          ...
                          <div className="absolute flex flex-col items-center justify-center pointer-events-none">
                            ...
                            <span className="text-[9px] uppercase font-bold text-ink-400">
                              {t('analytics.total')}
                            </span>
                          </div>
                        </div>
                ...
                <div className="bg-white border border-ink-100 p-6 rounded-xl shadow-sm flex flex-col justify-between">
                  <div>
                    <h3 className="text-base font-bold text-ink-800 mb-1">
                      {t('analytics.budgetLeaderboard')}
                    </h3>
                    <p className="text-xs text-ink-400 mb-6">
                      {t('analytics.budgetLeaderboardSub')}
                    </p>
                  ...
                  <div className="border-t pt-4 mt-6 text-center text-[10px] text-ink-400 font-bold uppercase tracking-wider">
                    {t('analytics.maxBudget', {
                      max: (
                        topProjectsLeaderboard[0]?.budget || 0
                      ).toLocaleString(),
                    })}
                  </div>
                </div>
                ...
                <div className="bg-white border border-ink-100 p-6 rounded-xl shadow-sm flex flex-col justify-between">
                  <div>
                    <h3 className="text-base font-bold text-ink-800 mb-1">
                      {t('analytics.skillDemand')}
                    </h3>
                    <p className="text-xs text-ink-400 mb-6">
                      {t('analytics.skillDemandSub')}
                    </p>

                    {topSkillsList.length === 0 ? (
                      <p className="text-xs text-ink-400 italic text-center py-8">
                        {t('analytics.noSkills')}
                      </p>
                    ) : (
                      <div className="w-full h-48">
                        <ResponsiveContainer width="100%" height="100%">
                          <BarChart
                            data={topSkillsList.map(([skill, count]) => ({
                              name: skill,
                              count: count,
                            }))}
                            layout="vertical"
                  ...
                  <div className="border-t pt-4 mt-6 text-center text-[10px] text-ink-400 font-bold uppercase tracking-wider">
                    {t('analytics.distinctRequirements', {
                      count: Object.keys(skillFrequency).length,
                    })}
                  </div>
                </div>
              </div>
