...
export default function CollegeDashboard() {const navigate = useNavigate();
 ...
 if (!savedUser.email || savedUser.userRole !=="college") {toast.error(t("college.sessionContextRequired"));
 navigate("/login");
 return;
}
 ...
 const handleEndorseToggle = async (studentEmail, currentStatus) => {try {const res = await fetch(`${API_BASE_URL}/api/college/endorse`, { credentials:"include",
 method:"POST",
 headers: {
"Content-Type":"application/json",
 ...
 if (res.ok) {toast.success(currentStatus ? t("college.endorsementRemoved") : t("college.endorsementSigned"));
 ...
 const handleCompanyToggle = async (companyEmail, status) => {try {const res = await fetch(`${API_BASE_URL}/api/college/toggle-company`, { credentials:"include",
 method:"POST",
 headers: {
"Content-Type":"application/json",
},
 ...
 if (res.ok) {toast.success(t("college.recruiterStatusUpdated", { status}));
 fetchDashboardData(currentUser.collegeName);
 ...
 const handleBulkImport = async (e) => {e.preventDefault();
 if (!bulkInput.trim()) {toast.error(t("college.enterStudentDataFirst"));
 return;
}

 setImporting(true);
 setImportStats(null);
 try {// Parse bulk text: Name, Email, Enrollment
 const lines = bulkInput.split("\n");
 const parsedStudents = [];
 for (const line of lines) {if (!line.trim()) continue;
 const parts = line.split(",");
 if (parts.length < 3) continue;
 parsedStudents.push({fullName: parts[0].trim(),
 email: parts[1].trim(),
 enrollmentNumber: parts[2].trim()
});
}

 if (parsedStudents.length === 0) {toast.error(t("college.noValidStudentRows"));
 setImporting(false);
 return;
}
...
"Content-Type":"application/json",
 ...
 if (res.ok) {toast.success(t("college.bulkImportComplete"));
 setImportStats({imported: data.importedCount,
 ...
 const majorStats = students.reduce((acc, student) => {const major = student.major ||"Computer Science";
 if (!acc[major]) {acc[major] = { major, totalScore: 0, count: 0, completedTasks: 0};
}
 ...
 const leaderboard = Object.values(majorStats).map(m => ({...m,
 ...
 <div>
 <h2 className="text-sm font-black text-ink-800 leading-tight">{t("college.academicControlPortal")}</h2>
 ...
 <div className="bg-gradient-to-r from-marigold-700 via-purple-700 to-marigold-600 rounded-xl shadow-sm p-6 sm:p-8 text-white mb-8 relative overflow-hidden">
 ...
 <div key={idx} className="bg-white rounded-xl shadow-sm border border-ink-100 p-4 hover:shadow-md transition">
 ...
 <p className="text-lg font-black text-ink-800 mt-0.5">{kpi.value}</p>
 </div>
 ))}
 </div>
 ...
 className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-bold transition whitespace-nowrap shrink-0 ${activeTab === tab.id
 ?"bg-marigold-500 text-white shadow-md shadow-marigold-100":"text-ink-600 hover:bg-ink-100"
}`}
 >
 <span>{tab.icon}</span>
 {tab.label}
 ...
 {students.map((student) => {const isAtRisk = student.readinessScore < 400;
 return (
 <tr key={student.email} className="hover:bg-ink-50 transition">
 <td className="px-6 py-4">
 <div>
 <p className="font-extrabold text-ink-800">{student.fullName}</p>
 <p className="text-[10px] text-ink-400 font-semibold">{student.email}</p>
 </div>
 </td>
 <td className="px-6 py-4">
 <div>
 <p className="font-bold text-ink-700">{student.major}</p>
 <p className="text-[10px] text-ink-400 font-semibold">{t("college.semesterLabel")}: {student.currentSemester}</p>
 </div>
 </td>
 <td className="px-6 py-4 text-center">
 <span className="font-black text-ink-700">{student.completedTasksCount}</span>
 </td>
 <td className="px-6 py-4 text-center">
 <div className="flex flex-col items-center">
 <span className={`font-black text-sm ${isAtRisk ?"text-red-500" : student.readinessScore >= 800 ?"text-emerald-600" :"text-marigold-500"
}`}>
 {student.readinessScore}
 </span>
 <span className={`text-[8px] font-black uppercase px-1 py-0.5 rounded mt-0.5 border ${isAtRisk ?"text-red-500 border-red-100 bg-red-50" : student.readinessScore >= 800 ?"text-emerald-500 border-emerald-100 bg-emerald-50" :"text-marigold-500 border-marigold-100 bg-marigold-50"
}`}>
 ...
 className={`px-3 py-1.5 rounded-xl text-[10px] font-bold border transition ${student.isEndorsed
 ?"bg-emerald-50 border-emerald-200 text-emerald-700":"bg-white hover:bg-ink-50 text-ink-500 border-ink-200"
}`}
 >
 ...
 <div className="flex items-center gap-4">
 <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white font-black text-sm ${idx === 0 ?"bg-amber-400 animate-pulse" : idx === 1 ?"bg-ink-400" :"bg-amber-700"
}`}>
 #{idx + 1}
 </div>
 <div>
 <h4 className="text-sm font-black text-ink-800">{dept.major}</h4>
 ...
 <td className="px-6 py-4">
 <p className="font-extrabold text-ink-800">{c.companyName}</p>
 </td>
 <td className="px-6 py-4">
 <p className="font-semibold text-ink-500">{c.email}</p>
 </td>
 <td className="px-6 py-4">
 <p className="font-bold text-ink-700">{c.industryVertical}</p>
 </td>
 <td className="px-6 py-4">
 <span className={`px-2 py-0.5 rounded text-[10px] font-extrabold border uppercase ${c.status ==="Approved"?"bg-green-50 text-green-600 border-green-100": c.status ==="Blocked"?"bg-red-50 text-red-600 border-red-100":"bg-ink-50 text-ink-500 border-ink-100"
}`}>
 {c.status}
 </span>
 </td>
 ...
 <div className="bg-white rounded-xl shadow-sm border border-ink-100 p-6">
 <div className="mb-5">
 <h3 className="text-sm font-black text-ink-800 uppercase tracking-wider">➕ {t("college.bulkStudentRegistrar")}</h3>
 <p className="text-[11px] text-ink-400 mt-1">
 {t("college.onboardingDesc1")} <code className="bg-ink-100 px-1 py-0.5 rounded font-mono font-bold text-ink-700">password123</code>.
 </p>
 </div>
 ...
 className="w-full bg-ink-50 border border-ink-200 p-4 rounded-xl text-xs outline-none focus:ring-2 focus:ring-marigold-400 focus:bg-white transition resize-none font-mono"placeholder={t("college.csvPlaceholder")}
 />
 </div>
 ...
 <p className="text-marigold-900/80">🔹 {t("college.duplicatesSkipped")}: <strong className="text-ink-600">{importStats.duplicate}</strong></p>
 ...
 <h2 className="text-xl font-black text-ink-800 tracking-tight">{t("college.academicCorpReportCard")}</h2>
 ...
 <p className="font-extrabold text-ink-800 mt-0.5">{printStudent.fullName}</p>
 </div>
 <div>
 <p className="text-ink-400 font-bold text-[9px] uppercase">{t("college.enrollmentNumber")}</p>
 <p className="font-extrabold text-ink-800 mt-0.5">{printStudent.enrollmentNumber}</p>
 </div>
 <div>
 <p className="text-ink-400 font-bold text-[9px] uppercase">{t("college.departmentMajor")}</p>
 <p className="font-extrabold text-ink-800 mt-0.5">{printStudent.major}</p>
 </div>
 <div>
 <p className="text-ink-400 font-bold text-[9px] uppercase">{t("college.currentSemester")}</p>
 <p className="font-extrabold text-ink-800 mt-0.5">{printStudent.currentSemester}</p>
 </div>
 </div>
 ...
 <p className="text-lg font-black text-ink-800 mt-0.5">{printStudent.completedTasksCount}</p>
 </div>
 <div className="bg-ink-50 p-3 rounded-xl border border-ink-100">
 <p className="text-[8px] font-black text-ink-400 uppercase tracking-wider">{t("college.avgRating")}</p>
 <p className="text-lg font-black text-ink-800 mt-0.5">{printStudent.avgRating}★</p>
 </div>
 <div className="bg-ink-50 p-3 rounded-xl border border-ink-100">
 ...
 <div className={`p-4 rounded-xl border text-center ${printStudent.isEndorsed
 ?"bg-emerald-50 border-emerald-200 text-emerald-800":"bg-amber-50 border-amber-200 text-amber-800"
}`}>
 <p className="text-xs font-black">
 {printStudent.isEndorsed
 ?`✓ ${t("college.facultyEndorsedDesc")}`:`⏳ ${t("college.facultyEndorsementPending")}`}
 </p>
 </div>
