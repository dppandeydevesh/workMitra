...
export default function ResetPasswordPage() {const { token} = useParams();
 ...
 const handleResetSubmit = async (e) => {e.preventDefault();
 setErrorMessage("");
 setSuccessMessage("");

 if (password !== confirmPassword) {setErrorMessage(t("resetPassword.errorMismatch"));
 return;
}

 if (passwordStrength.score < 4) {setErrorMessage(t("resetPassword.errorWeakPassword"));
 return;
}

 setSubmitting(true);
 try {const response = await fetch(`${API_BASE_URL}/api/auth/reset-password/${token}`, { credentials:"include",
 method:"POST",
 ...
 <div>
 <h2 className="text-2xl font-black text-purple-950">{t("resetPassword.heading")}</h2>
 <p className="text-xs text-ink-400 mt-1">{t("resetPassword.subheading")}</p>
 </div>
 ...
 <input
 type="password"placeholder={t("resetPassword.newPasswordPlaceholder")}
 value={password}
 onChange={(e) => checkPasswordStrength(e.target.value)}
 className="w-full pl-11 bg-purple-50/60 border border-purple-100 text-sm px-4 py-2.5 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition-all"required
 />
 </div>
 {password && (
 ...
 <Lock className="absolute left-4 top-1/2 transform -translate-y-1/2 text-ink-400 w-4 h-4" />
 <input
 type="password"placeholder={t("resetPassword.confirmPasswordPlaceholder")}
 ...
 <button
 type="submit"disabled={submitting || passwordStrength.score < 4}
 className={`w-full text-white text-xs font-bold uppercase tracking-wider py-3.5 rounded-xl shadow-md mt-2 transition ${passwordStrength.score === 4 ?"bg-gradient-to-r from-purple-600 to-pink-600 hover:opacity-95 cursor-pointer" :"bg-ink-300 cursor-not-allowed"}`}
 >
