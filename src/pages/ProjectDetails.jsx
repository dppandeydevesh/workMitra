...
export default function ProjectDetails() {
  ...
  const fetchProjectAndAppState = useCallback(async (userEmail) => {
    ...
    try {
      ...
      if (projectRes.ok) {
        ...
        setErrorMessage(projectData.error || t("projectDetails.failedToLoadDetails"));
      }
  ...
  const handleApply = () => {
    ...
    if (project.minReadinessScore && readinessScore < project.minReadinessScore) {
      toast.error(t("projectDetails.prerequisiteMismatch", { minScore: project.minReadinessScore, score: readinessScore }));
      return;
    }
    ...
    if (project.targetUniversity && !currentUser.email.toLowerCase().endsWith(project.targetUniversity.toLowerCase())) {
      toast.error(t("projectDetails.broadcastMismatch", { university: project.targetUniversity }));
      return;
    }
  ...
  if (errorMessage || !project) {
    return (
      <div className="min-h-screen dark:bg-slate-950 dark:bg-none bg-transparent flex items-center justify-center px-4">
