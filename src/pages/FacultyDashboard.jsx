...
export default function FacultyDashboard() {const { t} = useTranslation();
 ...
 <div className="text-center py-12 text-ink-400 font-bold">{t("facultyDashboard.loading") ||"Loading your projects..."}</div>
     ...
     <div className="wm-panel p-[40px_24px] text-center max-w-md mx-auto my-6 flex flex-col items-center justify-center">
       ...
       <div>
         <h3 className="text-[16px] font-medium text-[#1B2333] mb-[6px]">{t("facultyDashboard.noProjects") || "No projects posted yet"}</h3>
         <p className="text-[13px] text-[#6B7280] leading-[1.65] max-w-[260px] mx-auto">
           {t("facultyDashboard.noProjectsDesc") || "Post research tasks or class gig deployments to invite student submissions."}
         </p>
       </div>
     </div>
   ) : (
 ...
 {t("facultyDashboard.viewApplicants") || "View Applicants 👨‍🎓"}
 </button>
 </div>
 </div>
 ))}
 ...
 {activeTab ==="new-project" && (
  ...
  <h2 className="text-2xl font-black text-ink-800 mb-6">{t("facultyDashboard.createProject") || "Create Academic Project"}</h2>
  <form onSubmit={handlePostProject} className="space-y-5">
  <div>
  <label className="block text-sm font-extrabold text-ink-700 mb-2">{t("facultyDashboard.projectTitle") || "Project Title"}</label>
  <input
  type="text"required
  value={formData.title}
  onChange={(e) => setFormData({...formData, title: e.target.value})}
  className="w-full bg-ink-50 border border-ink-200 rounded-xl px-4 py-3 text-ink-800 focus:ring-2 focus:ring-marigold-500 outline-none transition"placeholder={t("facultyDashboard.projectTitlePlaceholder") || "e.g., Deep Learning Research Assistant"}/>
  </div>
  
  <div>
  <label className="block text-sm font-extrabold text-ink-700 mb-2">{t("facultyDashboard.detailedDescription") || "Detailed Description"}</label>
  <textarea
  required
  rows="4"value={formData.description}
  onChange={(e) => setFormData({...formData, description: e.target.value})}
  className="w-full bg-ink-50 border border-ink-200 rounded-xl px-4 py-3 text-ink-800 focus:ring-2 focus:ring-marigold-500 outline-none transition"placeholder={t("facultyDashboard.descriptionPlaceholder") || "Describe the research goals and student responsibilities..."}/>
  </div>

  <div>
  <label className="block text-sm font-extrabold text-ink-700 mb-2">{t("facultyDashboard.requiredSkills") || "Required Skills (Comma separated)"}</label>
  <input
  type="text"required
  value={formData.skills}
  onChange={(e) => setFormData({...formData, skills: e.target.value})}
  className="w-full bg-ink-50 border border-ink-200 rounded-xl px-4 py-3 text-ink-800 focus:ring-2 focus:ring-marigold-500 outline-none transition"placeholder={t("facultyDashboard.skillsPlaceholder") || "e.g., Python, PyTorch, Data Analysis"}/>
  </div>

  <div>
  <label className="block text-sm font-extrabold text-ink-700 mb-2">{t("facultyDashboard.incentive") || "Incentive (Stipend / Marks)"}</label>
  <input
  type="text"required
  value={formData.budget}
  onChange={(e) => setFormData({...formData, budget: e.target.value})}
  className="w-full bg-ink-50 border border-ink-200 rounded-xl px-4 py-3 text-ink-800 focus:ring-2 focus:ring-marigold-500 outline-none transition"placeholder={t("facultyDashboard.incentivePlaceholder") || "e.g., 50 Internal Marks"}/>
  </div>
  ...
  {posting ? (t("facultyDashboard.publishing") || "Publishing to Network...") : (t("facultyDashboard.publishProject") || "Publish Academic Project 🚀")}
  </button>
  </form>
  </div>
 )}
 ...
 ← Back to Projects
  ...
  <button onClick={() => setActiveTab("my-projects")} className="text-marigold-500 font-bold text-sm hover:underline">
    {t("facultyDashboard.backToProjects") || "← Back to Projects"}
  </button>
  </div>

  {loading ? (
  <div className="text-center py-12 text-ink-400 font-bold">{t("facultyDashboard.loadingApplicants") || "Loading applicants..."}</div>
  ) : applicants.length === 0 ? (
      <div className="wm-panel p-[40px_24px] text-center max-w-md mx-auto my-6 flex flex-col items-center justify-center">
        <div className="w-[48px] h-[48px] rounded-xl bg-[#FBE7C4] flex items-center justify-center text-[#F5A623] shadow-sm mb-4">
          <Users size={24} />
        ...
        <div>
          <h3 className="text-[16px] font-medium text-[#1B2333] mb-[6px]">{t("facultyDashboard.noApplicants") || "No applicants yet"}</h3>
          <p className="text-[13px] text-[#6B7280] leading-[1.65] max-w-[260px] mx-auto">
            {t("facultyDashboard.noApplicantsDesc") || "Students will appear here once they submit their interest or quiz results for review."}
          </p>
        </div>
      </div>
    ) : (
  ...
  <span className="text-[10px] font-bold text-ink-400 block uppercase">{t("facultyDashboard.atsMatch") || "ATS Match"}</span>
