...
export default function ProjectDetails() {
  ...
  const fetchProjectAndAppState = useCallback(
    async (userEmail) => {
      ...
      try {
        ...
        console.error(err);
        setErrorMessage(t('projectDetails.errorConnecting'));
      } finally {
        setLoading(false);
      }
  ...
  const handleApply = () => {
    ...
    const completedCount = studentApps.filter(
      (a) => a.status === 'Completed'
    ).length;
    const ratingApps = studentApps.filter(
    ...
    const readinessScore = Math.min(
      1000,
      Math.round(
        200 + completedCount * 150 + avgRating * 80 + profileCompleteness * 100
      )
    );

    // Prerequisite score tier check
    if (
      project.minReadinessScore &&
      readinessScore < project.minReadinessScore
    ) {
      toast.error(
        t('projectDetails.prerequisiteMismatch', {
          minScore: project.minReadinessScore,
    ...
    if (
      ...
      !currentUser.email
        .toLowerCase()
        .endsWith(project.targetUniversity.toLowerCase())
    ) {
      toast.error(
        t('projectDetails.broadcastMismatch', {
          university: project.targetUniversity,
        })
      );
      return;
    }
    ...
    if (
      project.preTestQuestions &&
      project.preTestQuestions.length > 0 &&
      !showQuizModal &&
  ...
  const executeApplyAPI = async () => {
    ...
    try {
      ...
      if (res.ok) {
        toast.success(t('projectDetails.applicationSubmitted'));
        track('application_submitted', { projectId, role: 'student' });
        setApplicationStatus('Pending');
      } else {
        toast.error(data.error || t('projectDetails.failedToSubmit'));
      }
    } catch (err) {
      console.error(err);
      toast.error(t('projectDetails.errorCommunicating'));
    } finally {
      setApplying(false);
  ...
  const handleWithdraw = async () => {
    if (!window.confirm(t('projectDetails.confirmWithdraw'))) return;

    const existingApp = studentApps.find(
      (app) => app.projectId?._id === projectId
    );
    ...
    try {
      const res = await fetchWithAuth(
        `${API_BASE_URL}/api/applications/withdraw/${existingApp._id}`,
        { method: 'PUT' }
      );
      const data = await res.json();
      if (res.ok) {
        toast.success(t('projectDetails.applicationWithdrawn'));
        setApplicationStatus('Withdrawn');
      } else {
        toast.error(data.error || t('projectDetails.failedToWithdraw'));
      }
    } catch (err) {
      console.error(err);
      toast.error(t('projectDetails.errorCommunicating'));
      ...
      <div className="min-h-screen bg-transparent flex items-center justify-center px-4">
        <div className="max-w-md w-full bg-white rounded-xl shadow-sm p-8 border border-ink-100 text-center">
          ...
          <h2 className="text-lg font-bold text-ink-800 mb-2">
            {t('projectDetails.failedToLoadTitle')}
          </h2>
          <p className="text-sm text-ink-500 mb-6">
            {errorMessage || t('projectDetails.couldNotBeFound')}
          </p>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 sm:p-8 border border-ink-100">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center border-b pb-6 mb-6 gap-4">
            <div>
              ...
              {project.hasPpiBadge && (
                <span className="ml-2 px-2 py-0.5 bg-gradient-to-r from-paper to-pink-600 text-white text-[8px] font-extrabold uppercase rounded-md tracking-wider">
              ...
              {project.targetUniversity && (
                <span className="ml-2 px-2 py-0.5 bg-amber-50 border border-amber-200 text-amber-700 text-[8px] font-extrabold uppercase rounded-md tracking-wider">
                  🎓{' '}
                  {t('projectDetails.universityOnly', {
                    university: project.targetUniversity,
                  })}
                </span>
              )}
              ...
              <p className="text-xs text-ink-400 mt-1 font-semibold uppercase tracking-wider">
                {t('projectDetails.postedBy')}{' '}
                {project.companyId?.email || project.companyId} •{' '}
                <span className="text-marigold-500">
                  {project.departmentName || t('projectDetails.coreTeam')}
                </span>
              </p>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            <div className="bg-ink-50 border border-ink-100 p-4 rounded-xl shadow-inner text-left">
              <span className="text-[9px] font-black text-ink-400 uppercase tracking-wider block">
                {t('projectDetails.timelineDuration')}
              </span>
              <span className="text-xs font-extrabold text-ink-800 mt-1 block">
                ⏱️ {project.duration || t('projectDetails.na')}
              </span>
            </div>

            <div className="bg-ink-50 border border-ink-100 p-4 rounded-xl shadow-inner text-left">
              <span className="text-[9px] font-black text-ink-400 uppercase tracking-wider block">
                {t('projectDetails.timeRemaining')}
              </span>
              <span className="text-xs font-extrabold text-ink-800 mt-1 block">
                ⏱️{' '}
                {(() => {
                  ...
                  if (diff <= 0)
                    return (
                      <span className="text-red-500 font-black">
                        {t('projectDetails.expired')}
                      </span>
                    );
                  const days = Math.ceil(diff / (1000 * 60 * 60 * 24));
                  return (
                    <span className="text-marigold-500 font-black">
                      {t('projectDetails.daysLeft', { days })}
                    </span>
                  );
                })()}
              </span>
            ...
            <div className="bg-ink-50 border border-ink-100 p-4 rounded-xl shadow-inner text-left">
              <span className="text-[9px] font-black text-ink-400 uppercase tracking-wider block">
                {t('projectDetails.studentCapacity')}
              </span>
              <span className="text-xs font-extrabold text-ink-800 mt-1 block">
                👥{' '}
                {t('projectDetails.slots', {
                  slots: project.studentsNeeded || 1,
                })}
              </span>
            </div>
            ...
            <div className="bg-ink-50 border border-ink-100 p-4 rounded-xl shadow-inner text-left">
              <span className="text-[9px] font-black text-ink-400 uppercase tracking-wider block">
                {t('projectDetails.complexityTier')}
              </span>
              <span className="text-xs font-extrabold text-ink-800 mt-1 block">
                🏷️ {project.complexity || t('projectDetails.intermediate')}
              </span>
            </div>
          </div>
          ...
          {/* Description */}
          <div className="mb-8">
            <h3 className="text-sm font-black text-ink-400 uppercase tracking-wider mb-3">
              {t('projectDetails.gigDetailsContext')}
            </h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
            <div>
              <h3 className="text-sm font-black text-ink-400 uppercase tracking-wider mb-3">
                {t('projectDetails.requiredTechStack')}
              </h3>
              <div className="flex flex-wrap gap-2">
                {requiredSkills.map((skill) => {
                  const matches = studentSkills.some(
                    (studentSkill) =>
            ...
            <div className="bg-marigold-50/50 border border-marigold-100/50 p-5 rounded-xl">
              <h3 className="text-sm font-black text-marigold-900/50 uppercase tracking-wider mb-2.5">
                {t('projectDetails.aiSkillsMatchRating')}
              </h3>
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-xs font-bold text-ink-700">
                    {t('projectDetails.youMatchSkills', {
          ...
          <div className="border-t pt-6 flex flex-col sm:flex-row justify-between items-center gap-4">
              ...
              {applicationStatus ? (
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-extrabold text-ink-600">
                    ...
                    <span
                      className={`ml-1 px-2.5 py-0.5 rounded-lg border text-[10px] font-black uppercase ${
                        applicationStatus === 'Approved'
                          ? 'bg-emerald-50 border-emerald-100 text-emerald-800'
                          : applicationStatus === 'Rejected'
                            ? 'bg-rose-50 border-rose-100 text-rose-800'
                            : applicationStatus === 'Completed'
                              ? 'bg-marigold-50 border-marigold-100 text-marigold-800'
                ...
                <p className="text-xs text-ink-400 italic">
                  {t('projectDetails.ensureProfileConfigured')}
                </p>
              )}
            </div>
            ...
            <div className="flex gap-3">
              ...
              {!applicationStatus && (
                ...
                >
                  {applying
                    ? t('projectDetails.submittingPayload')
              ...
              {(applicationStatus === 'Pending' ||
                ...
                >
                  {applying
                    ? t('projectDetails.withdrawing')
                    : t('projectDetails.withdrawApplication')}
                </button>
              )}
            </div>
          </div>
            ...
            <div className="bg-white rounded-xl max-w-lg w-full shadow-sm p-6 border text-left flex flex-col animate-fade-in">
              <div className="flex justify-between items-center border-b pb-3 mb-4">
                <h3 className="font-black text-ink-800 text-sm uppercase tracking-wider">
                  🔒 {t('projectDetails.ndaModalTitle')}
                </h3>
                <button
              ...
              <div className="bg-ink-50 border p-4 rounded-xl text-xs text-ink-600 leading-relaxed max-h-60 overflow-y-auto space-y-2">
                <p className="font-extrabold text-ink-800 text-[13px]">
                  {t('projectDetails.ndaRulesTitle')}
                </p>
              ...
              <div className="flex justify-end gap-2.5 pt-4 border-t mt-4">
                ...
                <button
                  ...
                  onClick={() => {
                    if (!ndaAccepted) {
                      toast.error(t('projectDetails.acceptNdaError'));
                      return;
                    }
                    setShowNdaModal(false);
                    handleApply();
                ...
                >
                  {applying
                    ...
                    : t('projectDetails.completeApplication')}
                </button>
              </div>
            </div>
          </div>
          ...
          project.preTestQuestions.length > 0 && (
            <div className="fixed inset-0 bg-ink-900/60 z-50 flex items-center justify-center p-4">
              <div className="bg-white rounded-xl max-w-lg w-full shadow-sm p-6 border text-left flex flex-col animate-fade-in">
                <div className="flex justify-between items-center border-b pb-3 mb-4">
                  <h3 className="font-black text-ink-800 text-sm uppercase tracking-wider">
                    📝 {t('projectDetails.quizModalTitle')}
                  </h3>
                  <button
                ...
                <p className="text-xs text-ink-500 mb-4 font-semibold">
                  {t('projectDetails.quizIntro')}
                </p>
                ...
                <div className="space-y-4 max-h-80 overflow-y-auto pr-1">
                  {project.preTestQuestions.map((q, qIdx) => (
                    <div
                      key={qIdx}
                      className="border p-4 rounded-xl bg-ink-50 space-y-2"
                    >
                ...
                <div className="flex justify-end gap-2.5 pt-4 border-t mt-4">
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      if (
                        ...
                        project.preTestQuestions.length
                      ) {
                        toast.error(t('projectDetails.answerAllQuestions'));
                        return;
                      }

                      if (!allCorrect) {
                        toast.error(t('projectDetails.screeningFailed'));
                        return;
                      }
                  ...
                  >
                    {applying
                      ? t('projectDetails.submitting')
                      : t('projectDetails.submitAnswers')}
                  </button>
                </div>
              </div>
            </div>
