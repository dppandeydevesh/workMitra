...
import {Briefcase, CheckCircle, MessageSquare, Ticket, Inbox, 
...
export default function Dashboard() {const navigate = useNavigate();
 ...
 const renderStepper = useCallback((status) => {const steps = [
 ...
 <span className={`px-1.5 py-0.5 rounded font-extrabold transition-all uppercase ${isActive ?"bg-gradient-to-r from-purple-600 to-pink-600 text-white shadow-sm" :
...
"bg-ink-100 text-ink-400 font-normal"
 ...
 const scrollToSection = (id) => {const element = document.getElementById(id);
 ...
 useEffect(() => {if (activeAppToSubmit) {const draft = JSON.parse(localStorage.getItem(`workmitra_draft_${activeAppToSubmit._id}`) ||"{}");
 ...
 if (file.type !=="application/pdf") {toast.error(t("dashboard.pdfFormatError"));
 return;
}
 ...
 try {const response = await fetchWithAuth(`${API_BASE_URL}/api/profile/upload-cv`, { credentials:"include",
 method:"POST",
 headers: {
},
 body: formData
 ...
 if (response.ok) {setResumeText(data.resumeText);
...
} else {toast.error(data.error || t("dashboard.cvUploadFail"));
}
} catch (err) {toast.error(t("dashboard.uploadGatewayError"));
} finally {setUploadingCV(false);
 e.target.value = null; // Clear the input so the same file can be uploaded again if needed
}
};

 const handleUpdateResumeDetails = async (e) => {e.preventDefault();
 if (!currentUser) return;
 setUpdatingResume(true);

 try {const response = await fetchWithAuth(`${API_BASE_URL}/api/profile/resume`, { credentials:"include",
 method:"POST",
 ...
 const data = await response.json();
 if (response.ok) {toast.success(t("dashboard.resumeUpdateSuccess"));
 const updatedUser = {...currentUser,
 ...
 setCurrentUser(updatedUser);
 localStorage.setItem("user", JSON.stringify(updatedUser));
} else {toast.error(data.error || t("dashboard.resumeUpdateFail"));
}
} catch (err) {toast.error(t("dashboard.serverGatewayError"));
} finally {setUpdatingResume(false);
}
};

 const handleReviewCVWithAI = async () => {if (!currentUser) return;
 if (!resumeText || resumeText.trim().length === 0) {toast.info(t("dashboard.pasteCvInfo"));
 return;
}
 setLoadingReview(true);

 try {const response = await fetchWithAuth(`${API_BASE_URL}/api/ai/review-cv`, { credentials:"include",
 method:"POST",
 headers: {"Content-Type":"application/json",
},
 body: JSON.stringify({email: currentUser.email,
 ...
 const data = await response.json();
 if (response.ok) {setCvReport(data.report);
 const updatedUser = {...currentUser,
 resumeText,
 ...
 setCurrentUser(updatedUser);
 localStorage.setItem("user", JSON.stringify(updatedUser));
} else {toast.error(data.error || t("dashboard.cvReviewFail"));
}
} catch (err) {toast.error(t("dashboard.aiEngineError"));
} finally {setLoadingReview(false);
}
};

 const fetchApplications = async (userEmail) => {try {const detailsRes = await fetchWithAuth(`${API_BASE_URL}/api/applications/student-details/${userEmail}`, { credentials:"include",
 headers: {}
});
 if (detailsRes.ok) {const details = await detailsRes.json();
 setMyApplications(details);
}
} catch (err) {console.error("Failed to refetch details");
}
};

 const handleSubmitWork = async (e) => {e.preventDefault();
 if (!activeAppToSubmit) return;

 try {const response = await fetchWithAuth(`${API_BASE_URL}/api/applications/${activeAppToSubmit._id}/submit`, { credentials:"include",
 method:"POST",
 headers: {"Content-Type":"application/json",
},
 ...
 githubRepoUrl,
 liveDeploymentUrl,
 aiDeclaration: {usedAi,
 aiPercentage,
 toolsUsed: aiToolsUsed
},
 selfAssessment: {codeQuality: codeQualityRubric,
 ...
 const data = await response.json();
 if (response.ok) {toast.success(t("dashboard.workSubmitSuccess"));
 ...
 if (currentUser) {fetchApplications(currentUser.email);
}
} else {toast.error(data.error || t("dashboard.submissionFail"));
}
} catch (err) {toast.error(t("dashboard.submissionPayloadError"));
}
};

 const handleRequestExtensionSubmit = async (e) => {e.preventDefault();
 if (!activeAppToExtend) return;

 setRequestingExtension(true);
 try {const res = await fetchWithAuth(`${API_BASE_URL}/api/applications/${activeAppToExtend._id}/request-extension`, { credentials:"include",
 method:"POST",
 headers: {
"Content-Type":"application/json",
 ...
 if (res.ok) {toast.success(t("dashboard.extensionSuccess"));
 setShowExtensionModal(false);
 setExtensionReason("");
 if (currentUser) {fetchApplications(currentUser.email);
}
} else {toast.error(data.error || t("dashboard.extensionFail"));
}
} catch (err) {toast.error(t("dashboard.extensionNetworkError"));
} finally {setRequestingExtension(false);
}
};

 // Dynamic application notifications log feed
 const notificationsList = myApplications.map(app => {const project = app.projectId;
 if (!project) return null;
 if (app.status ==="Approved") {return {id: app._id,
 ...
 if (app.status ==="Completed") {return {id: app._id,
 ...
 if (app.status ==="Rejected") {return {id: app._id,
 ...
 const filteredProjects = projects.filter((project) => {const query = searchTerm.toLowerCase();
 const matchesSearch = 
...
}).sort((a, b) => {if (sortBy ==="budgetHigh") return (b.budget || 0) - (a.budget || 0);
 if (sortBy ==="closestDeadline") {const dA = a.deadline ? new Date(a.deadline).getTime() : Infinity;
                    ...
                    <div className="flex items-start gap-2.5 mt-3 select-none">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-ink-500 mt-1 font-medium leading-none">
                          {currentUser.collegeName} {currentUser.enrollmentNumber ? `· ID: ${currentUser.enrollmentNumber}` : ''}
                        </p>
                      </div>
                    </div>
                  )}
 ...
 <h3 className="font-bold text-lg text-marigold-950 mb-2 group-hover:text-marigold-700 flex items-center gap-2"><Briefcase className="w-5 h-5" /> {t("dashboard.myGigs")}</h3>
 <p className="text-ink-600 text-xs">{t("dashboard.manageGigs")}</p>
 </motion.button>
 <motion.button 
 variants={itemVariants}
 ...
 <h3 className="font-bold text-lg text-green-950 mb-2 group-hover:text-green-700 flex items-center gap-2"><CheckCircle className="w-5 h-5" /> {t("dashboard.orders")}</h3>
 <p className="text-ink-600 text-xs">{t("dashboard.exploreProjects")}</p>
 </motion.button>
 <motion.button 
 variants={itemVariants}
 onClick={() => navigate("/chat")}
 className="group text-left bg-purple-50 p-4 sm:p-6 rounded-xl hover:bg-purple-100/70 hover:-translate-y-1 transition-all duration-300 shadow-sm border border-purple-100/30 wm-panel wm-card">
 <h3 className="font-bold text-lg text-purple-950 mb-2 group-hover:text-purple-700 flex items-center gap-2"><MessageSquare className="w-5 h-5" /> {t("dashboard.messages")}</h3>
 <p className="text-ink-600 text-xs">{t("dashboard.chatManagers")}</p>
 </motion.button>
 </div>
 ...
 <div id="gigs-section" className="mt-12 pt-8 border-t border-ink-100">
 <h2 className="text-lg sm:text-2xl font-bold text-ink-800 mb-2">{t("dashboard.myGigsAppsTitle")}</h2>
 ...
 <h4 className="text-sm font-bold text-ink-900 mb-1">{project.title}</h4>
 ...
 <p className="text-xs text-ink-600 line-clamp-2 leading-relaxed mb-4">{project.description}</p>
 </div>
 ...
 <strong>{t("dashboard.revisionRequested")}</strong>"{app.feedbackText}"</div>
 )}
 ...
 <span className={`font-black uppercase text-[9px] ${app.extensionRequests[app.extensionRequests.length - 1].status ==="Approved"?"text-green-600": app.extensionRequests[app.extensionRequests.length - 1].status ==="Rejected"?"text-red-500":"text-amber-600 animate-pulse"
}`}>
...
"{app.feedbackText}"</p>
 ...
 <input
 type="file"accept=".pdf"onChange={handleUploadCVFile}
 ...
 <div className="text-[10px] font-bold text-ink-700">{t("dashboard.dragDropPdf")}</div>
 ...
 <button
 type="button"onClick={handleReviewCVWithAI}
 disabled={loadingReview || !resumeText}
 className={`w-full py-3 text-white font-bold text-xs uppercase tracking-wider rounded-xl transition shadow-md ${!resumeText || loadingReview 
 ?"bg-ink-300 cursor-not-allowed":"bg-marigold-500 hover:bg-marigold-600 active:scale-[0.98]"
}`}
 >
 ...
 {/* ========================================================================= */}
 <div id="projects-section" className="mt-12 pt-8 border-t border-ink-100">
 <h2 className="text-lg sm:text-2xl font-bold text-ink-800 mb-2">{t("dashboard.deployedCompanyProjects")}</h2>
 ...
 {[1, 2, 3].map(i => (
 <div key={i} className="min-w-[280px] sm:min-w-[320px] h-32 rounded-xl border border-ink-200 skeleton-loader wm-panel"></div>
 ))}
 </div>
 ) : aiTopPicks.length > 0 ? (
 ...
 <input
 type="text"placeholder={t("dashboard.searchPlaceholder")}
 value={searchTerm}
 ...
 const suggestions = [];
 projects.forEach(p => {if (p.title && p.title.toLowerCase().includes(query) && !suggestions.includes(p.title)) {suggestions.push(p.title);
}
 if (p.requiredSkills) {p.requiredSkills.forEach(s => {if (s.toLowerCase().includes(query) && !suggestions.includes(s)) {suggestions.push(s);
}
});
}
});
 ...
 <button
 key={idx}
 type="button"onClick={() => { setSearchTerm(suggestion); setShowSearchSuggestions(false);}}
 ...
 <input
 type="range"min="1000"max="10000"step="1000"value={maxBudgetFilter}
 ...
 {[1, 2, 3, 4, 5, 6].map(i => (
 <div key={i} className="rounded-xl border border-ink-200 p-4 sm:p-6 shadow-sm h-48 skeleton-loader wm-panel"></div>
 ))}
 </div>
    ...
    <div className="text-center py-12 px-6 border border-ink-200 rounded-xl bg-white max-w-md mx-auto my-6 flex flex-col items-center justify-center gap-4">
      ...
      <div>
        <h3 className="text-sm font-semibold text-ink-900 tracking-tight">{t("No matching projects yet")}</h3>
        <p className="text-xs text-ink-500 mt-1 max-w-xs leading-relaxed">
          {t("Projects from verified companies that match your skills appear here. Set your preferences to get started.")}
        </p>
      </div>
      <button
 ...
 <span key={index} className="bg-ink-50 text-ink-600 text-[10px] font-medium px-2 py-0.5 rounded border border-ink-100">
 {skill}
 </span>
 ))}
 </div>
 ...
 <button className="bg-ink-100 text-ink-500 font-bold text-xs px-4 py-2 rounded-lg cursor-not-allowed border" disabled>
 ...
 disabled={page === 1}
 className={`px-4 py-2 rounded-xl text-xs font-bold transition shadow-sm ${page === 1 
 ?"bg-ink-100 text-ink-400 cursor-not-allowed":"bg-white border border-ink-200 text-ink-700 hover:bg-ink-50"
}`}
 >
 {t("dashboard.previousBtn")}
 </button>
 <span className="text-xs font-bold text-ink-600">{t("dashboard.pageText", { page})}</span>
 <button
 onClick={() => setPage(p => p + 1)}
 disabled={projects.length < limit}
 className={`px-4 py-2 rounded-xl text-xs font-bold transition shadow-sm ${projects.length < limit
 ?"bg-ink-100 text-ink-400 cursor-not-allowed":"bg-white border border-ink-200 text-ink-700 hover:bg-ink-50"
}`}
 >
 {t("dashboard.nextBtn")}
 ...
 <h3 className="font-black text-sm text-ink-900 uppercase tracking-wider">{t("dashboard.submitWorkTitle")} {activeAppToSubmit.projectId.title}</h3>
 <button onClick={() => setShowSubmitModal(false)} className="text-ink-400 hover:text-ink-600 text-lg font-black">×</button>
 </div>
 <form onSubmit={handleSubmitWork} className="space-y-4">
 <div>
 <label className="block text-[10px] font-bold text-ink-500 uppercase mb-1">{t("dashboard.submissionLinkLbl")}</label>
 <input
 type="url"placeholder="https://drive.google.com/..."value={submissionLink}
 ...
 <input
 type="url"placeholder="https://github.com/username/repo"value={githubRepoUrl}
 ...
 <input
 type="url"placeholder="https://project.vercel.app"value={liveDeploymentUrl}
 ...
 <input
 type="range"min="1"max="10"value={codeQualityRubric}
 ...
 <input
 type="range"min="1"max="10"value={correctnessRubric}
 ...
 <input
 type="range"min="1"max="10"value={documentationRubric}
 ...
 <div className="flex items-center gap-2">
 <input
 type="checkbox"id="usedAiCheckbox"checked={usedAi}
 ...
 <label htmlFor="usedAiCheckbox" className="text-xs font-bold text-ink-700 cursor-pointer select-none flex items-center gap-1.5">
 ...
 <input
 type="range"min="0"max="100"value={aiPercentage}
 ...
 <input
 type="text"placeholder={t("dashboard.aiToolsPlaceholder")}
 value={aiToolsUsed}
 ...
 <button
 type="submit"className="px-5 py-2 bg-gradient-to-r from-marigold-600 to-marigold-600 hover:opacity-95 text-white rounded-xl text-xs font-black transition shadow-md">
 ...
 <div className="flex justify-between items-center border-b pb-3 mb-4">
 <h3 className="font-bold text-base text-ink-900">{t("dashboard.requestDeadlineExt")}</h3>
 <button onClick={() => setShowExtensionModal(false)} className="text-ink-400 hover:text-ink-600 text-lg">×</button>
 </div>
 ...
 <input
 type="number"min="1"max="7"value={extensionDays}
 onChange={(e) => setExtensionDays(e.target.value)}
 className="w-full bg-ink-50 border border-ink-200 text-xs px-3.5 py-2.5 rounded-xl focus:outline-none"required
 />
 </div>
 <div>
 ...
 <div className="flex justify-end space-x-3 pt-2">
 <button
 type="button"onClick={() => setShowExtensionModal(false)}
 className="px-4 py-2 border border-ink-200 rounded-xl text-xs font-bold text-ink-500 hover:bg-ink-50 transition">
 {t("dashboard.cancelBtn")}
 </button>
 <button
