    ...
    <div className="min-h-screen bg-transparent py-8 font-sans">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8 bg-white p-4 sm:p-6 rounded-xl shadow-sm">
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-ink-800">
              {t('myProjects.title')}
            </h1>
          ...
          <button
            ...
            className="w-full sm:w-auto text-center text-xs font-bold px-4 py-2.5 rounded-xl transition shadow-sm"
          >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1 space-y-4">
            <h3 className="text-sm font-bold text-ink-700 uppercase tracking-wider mb-2 px-1">
              {t('myProjects.activeDeployments')}
            </h3>
            ...
            {loadingProjects ? (
              <div className="space-y-3">
                {[1, 2, 3].map((i) => (
                  <div
                    key={i}
                    className="rounded-xl border border-ink-200 p-5 shadow-sm h-24 skeleton-loader wm-panel"
                  ></div>
                ))}
              </div>
            ) : projects.length === 0 ? (
              <div className="wm-panel p-[48px_24px] text-center max-w-md mx-auto my-6 flex flex-col items-center justify-center">
                ...
                <div className="mt-4">
                  <h3 className="text-[16px] font-medium text-[#1B2333] mb-[6px]">
                    {t('No projects deployed yet')}
                  </h3>
                  <p className="text-[13px] text-[#6B7280] leading-[1.65] max-w-[260px] mx-auto">
                    {t(
                      'Deploy your first corporate gig to hire verified college students.'
                    )}
                  </p>
                </div>
                <button
              ...
              projects.map((proj) => (
                ...
                >
                  <h4 className="font-bold text-ink-900 text-sm mb-1">
                    {proj.title}
                  </h4>
                  <p className="text-[11px] text-ink-500 line-clamp-2 leading-relaxed mb-3">
                    {proj.description}
                  </p>
                  <div className="flex justify-between items-center text-[10px] text-ink-400 font-bold uppercase tracking-wide border-t pt-2.5">
                    ...
                    <span className="text-marigold-500">
                      {t('myProjects.inspectApplications')} →
                    </span>
                  </div>
                </div>
              ))
              ...
              <div className="bg-white rounded-xl shadow-sm p-6 border border-ink-100">
                <div className="border-b pb-4 mb-6">
                  <div className="flex justify-between items-start gap-4">
                    <div>
                      <span className="bg-marigold-100 text-marigold-800 text-[10px] font-bold px-2.5 py-1 rounded-md uppercase tracking-wide">
                        {t('myProjects.selectedStream')}
                      </span>
                      <h2 className="text-xl font-bold text-ink-900 mt-2">
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="px-2.5 py-1.5 border border-ink-200 text-ink-600 hover:bg-ink-50 rounded-lg text-xs font-bold transition flex items-center gap-1"
                      ...
                      {selectedProject.status !== 'Archived' && (
                        <button
                          onClick={() =>
                            handleArchiveProject(selectedProject._id)
                          }
                  ...
                  <p className="text-xs text-ink-500 mt-1.5">
                    {t('myProjects.duration')}: {selectedProject.duration} |{' '}
                    {t('myProjects.budget')}: ₹
                ...
                {loadingApplicants ? (
                  <div className="space-y-3 col-span-2">
                    {[1, 2, 3].map((i) => (
                      <div
                        key={i}
                        className="rounded-xl border border-[#E1E2DC] p-5 shadow-sm h-24 skeleton-loader wm-panel"
                      ></div>
                    ))}
                  </div>
                ) : applicants.length === 0 ? (
                    ...
                    <table className="w-full text-left text-xs text-ink-600">
                      <thead>
                        <tr className="bg-ink-50 uppercase text-[10px] font-bold text-ink-400 border-b tracking-wider">
                          ...
                          <th className="p-3">
                            {t('myProjects.skillsInventory')}
                          </th>
                          <th className="p-3 text-center">
                            {t('myProjects.smartMatch')}
                          </th>
                          <th className="p-3 text-center">
                            {t('myProjects.status')}
                          </th>
                          <th className="p-3 text-right">
                            {t('myProjects.actions')}
                          </th>
                        </tr>
                      </thead>
                        ...
                        {applicants.map((app) => (
                          ...
                          >
                            <td className="p-3">
                              ...
                              {app.resumeUrl ? (
                                <a
                                  ...
                                  className="text-[10px] text-marigold-500 font-bold hover:underline block mt-0.5"
                                >
                                  📄 {t('myProjects.viewResume')} ↗
                                </a>
                              ) : (
                                <span className="text-[9px] text-ink-400 italic block mt-0.5">
                                  {t('myProjects.noResumeProvided')}
                                </span>
                              )}
                              ...
                              <div className="flex items-center gap-2 mt-1 text-[9px]">
                                {app.githubUrl && (
                                  <a
                                    ...
                                    target="_blank"
                                    rel="noreferrer"
                                    className="text-ink-600 hover:text-ink-dark font-bold"
                                    title={t('myProjects.github')}
                                  >
                                ...
                                {app.linkedinUrl && (
                                  <a
                                    ...
                                    target="_blank"
                                    rel="noreferrer"
                                    className="text-marigold-500 hover:text-ink-dark font-bold"
                                    title={t('myProjects.linkedin')}
                                  >
                                ...
                                {app.portfolioUrl && (
                                  <a
                                    ...
                                    target="_blank"
                                    rel="noreferrer"
                                    className="text-green-600 hover:text-ink-dark font-bold"
                                    title={t('myProjects.portfolio')}
                                  >
                              ...
                              {app.aiRationale && (
                                <p
                                  className="text-[9px] text-ink-dark font-semibold leading-normal max-w-[150px] truncate mt-1"
                                  title={app.aiRationale}
                                >
                                  🤖 {app.aiRationale}
                                </p>
                              )}
                            ...
                            <td className="p-3 text-center">
                              <span
                                className={`px-2.5 py-1 rounded-full text-[11px] font-bold ${app.matchScore >= 75 ? 'bg-green-100 text-green-800' : app.matchScore >= 40 ? 'bg-amber-100 text-amber-800' : 'bg-ink-100 text-ink-600'}`}
                              >
                                {app.matchScore}%
                              </span>
                            </td>
                            ...
                            <td className="p-3 text-right space-x-2">
                              {app.status === 'Pending' && (
                                <>
                                  <button
                                    onClick={() =>
                                      handleAcceptApplicant(app.applicationId)
                                    }
                                    className="text-[10px] font-bold bg-green-600 hover:bg-green-700 text-white px-2 py-1 rounded transition shadow-sm"
                                  >
                                    {t('myProjects.accept')}
                                  </button>
                                  <button
                                    onClick={() =>
                                      handleRejectApplicant(app.applicationId)
                                    }
                                    className="text-[10px] font-bold bg-red-600 hover:bg-red-700 text-white px-2 py-1 rounded transition shadow-sm"
                                  >
                                    {t('myProjects.reject')}
                                  </button>
                                </>
                              ...
                              {app.status === 'Approved' && (
                                <span className="text-[11px] text-marigold-500 font-semibold">
                                  {t('myProjects.working')}
                                </span>
                              )}
                              {app.status === 'Submitted' && (
                                <button
                                  ...
                                  className="text-[10px] font-bold px-2.5 py-1 rounded transition shadow-sm"
                                >
                                  {t('myProjects.reviewWork')}
                                </button>
                              )}
                              {app.status === 'Completed' && (
                                <span className="text-[11px] text-green-600 font-bold block">
                                  ✓ {t('myProjects.approved')}
                                </span>
                              )}
                              {app.status === 'Rejected' && (
                                <span className="text-[11px] text-red-600 font-medium">
                                  {t('myProjects.rejected')}
                                </span>
                              )}
                            </td>
                          </tr>
              ...
              <div className="bg-white rounded-xl max-w-md w-full shadow-sm p-6 border animate-fade-in text-left">
                <div className="flex justify-between items-center border-b pb-3 mb-4">
                  <h3 className="font-bold text-base text-ink-900">
                    {t('myProjects.reviewWorkSubmission')}
                  </h3>
                  <button
                ...
                <div className="space-y-4">
                  <div>
                    <p className="text-[10px] uppercase font-bold text-ink-400">
                      {t('myProjects.candidate')}
                    </p>
                    <p className="text-xs font-bold text-ink-900">
                      {activeAppToReview.studentName} (
                      {activeAppToReview.studentEmail})
                    </p>
                  </div>
                  <div>
                    <p className="text-[10px] uppercase font-bold text-ink-400">
                      {t('myProjects.submissionLink')}
                    </p>
                    <a
                      href={activeAppToReview.submissionLink}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-xs text-marigold-500 font-bold hover:underline break-all"
                    >
                  ...
                  <div>
                    <p className="text-[10px] uppercase font-bold text-ink-400">
                      {t('myProjects.studentExplanationNotes')}
                    </p>
                  ...
                  >
                    <div>
                      ...
                      <textarea
                        ...
                        rows={2}
                        className="w-full bg-ink-50 border border-ink-200 text-xs px-3.5 py-2 rounded-xl focus:outline-none focus:ring-2 focus:ring-marigold-400 resize-none"
                        required
                      />
                    </div>
                    ...
                    <div className="border-t pt-2 space-y-2">
                      <div>
                        <label className="block text-[9px] font-extrabold text-ink-dark uppercase tracking-wider mb-0.5">
                          {t('myProjects.candidateRating')}
                        </label>
                        <div className="flex items-center gap-1">
                          ...
                          <span className="text-[9px] text-ink-400 font-extrabold ml-1.5">
                            ({rating} / 5)
                          </span>
                        </div>
                      </div>
                      ...
                      <div>
                        <label className="block text-[9px] font-extrabold text-ink-dark uppercase tracking-wider mb-0.5">
                          {t('myProjects.performanceReviewNotes')}
                        </label>
                        <textarea
                          ...
                          rows={1}
                          className="w-full bg-ink-50 border border-ink-200 text-xs px-3 py-1.5 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#F5A623] resize-none"
                        />
                      </div>
                    </div>
          ...
          {showEditModal && selectedProject && (
            <div className="fixed inset-0 bg-ink-900/50 z-50 flex items-center justify-center p-4">
              <div className="bg-white rounded-xl max-w-lg w-full shadow-sm p-6 border animate-fade-in text-left">
                <div className="flex justify-between items-center border-b pb-3 mb-4">
                  <h3 className="font-bold text-base text-ink-900 font-sans">
                    {t('myProjects.editProjectTitle')}
                  </h3>
                  <button
                ...
                <form onSubmit={handleSaveEditProject} className="space-y-4">
                  <div>
                    <label className="block text-[10px] font-bold text-ink-400 uppercase mb-1">
                      {t('myProjects.projectTitleLabel')}
                    </label>
                    <input
                      type="text"
                      value={editTitle}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-[10px] font-bold text-ink-400 uppercase mb-1">
                        {t('myProjects.budgetPayoutLabel')}
                      </label>
                      <input
                        type="number"
                        value={editBudget}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-[10px] font-bold text-ink-400 uppercase mb-1">
                        {t('myProjects.deadlineDateLabel')}
                      </label>
                      <input
                        type="text"
                        value={editDeadline}
                    ...
                    <div>
                      <label className="block text-[10px] font-bold text-ink-400 uppercase mb-1">
                        {t('myProjects.requiredSkillsLabel')}
                      </label>
                      <input
                        type="text"
                        value={editSkills}
