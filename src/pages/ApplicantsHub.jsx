    ...
    if (status === 'Disputed') {
      return (
        <span className="bg-rose-100 text-rose-800 text-[9px] font-black px-2.5 py-0.5 rounded-lg border border-rose-200 uppercase tracking-wider animate-pulse flex items-center gap-1">
          ...
          {t('applicantsHub.flaggedDisputed')}
        </span>
      );
    }
        ...
        {steps.map((step, idx) => {
          ...
          return (
            <div key={idx} className="flex items-center space-x-1">
              <span
                className={`px-1.5 py-0.5 rounded font-extrabold transition-all uppercase tracking-wider ${
                  isActive
              ...
              {idx < steps.length - 1 && (
                <span
                  className={
                    isCompleted ? 'text-ink-dark font-bold' : 'text-ink-300'
                  }
                >
                  ➔
                </span>
          ...
          <div className="border-b pb-6 mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              ...
              <p className="text-ink-500 mt-1 text-sm">
                {t('applicantsHub.commandCenterDesc')}
              </p>
            </div>
            <button
          ...
          <div className="wm-panel p-5 rounded-xl mb-8 grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <div className="md:col-span-1">
              <label className="block text-[10px] font-bold text-ink-400 uppercase mb-1.5">
                {t('applicantsHub.searchCandidate')}
              </label>
            ...
            <div>
              <label className="block text-[10px] font-bold text-ink-400 uppercase mb-1.5">
                {t('applicantsHub.filterByProject')}
              </label>
              <select
                value={projectFilter}
            ...
            <div>
              <label className="block text-[10px] font-bold text-ink-400 uppercase mb-1.5">
                {t('applicantsHub.filterByStatus')}
              </label>
              <select
                value={statusFilter}
              ...
              >
                ...
                <option value="Pending">
                  {t('applicantsHub.statusPending')}
                </option>
                <option value="Approved">
                  {t('applicantsHub.statusApproved')}
                </option>
                <option value="Submitted">
                  {t('applicantsHub.statusSubmitted')}
                </option>
                <option value="Completed">
                  {t('applicantsHub.statusCompleted')}
                </option>
                <option value="Rejected">
                  {t('applicantsHub.statusRejected')}
                </option>
              </select>
            </div>
            ...
            <div>
              <label className="block text-[10px] font-bold text-ink-400 uppercase mb-1.5">
                {t('applicantsHub.sortCandidatesBy')}
              </label>
              <select
                value={sortBy}
              ...
              >
                <option value="match">
                  {t('applicantsHub.sortMatchScore')}
                </option>
            ...
            <div className="flex items-center gap-2 pt-4">
              ...
              />
              <label
                htmlFor="blindModeToggle"
                className="text-[9px] font-black text-ink-500 uppercase cursor-pointer select-none flex items-center gap-1"
              >
          ...
          {loading ? (
            <div className="space-y-4">
              {[1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="skeleton-loader h-32 rounded-xl w-full wm-panel"
                ></div>
              ))}
            </div>
            ...
            <div className="wm-panel p-[40px_24px] text-center max-w-md mx-auto my-6 flex flex-col items-center justify-center">
              ...
              <div>
                ...
                <p className="text-[13px] text-[#6B7280] leading-[1.65] max-w-[260px] mx-auto">
                  No students have applied to this project yet. Share it with
                  your network to attract candidates.
                </p>
              </div>
            </div>
          ) : (
                ...
                >
                  ...
                  <div className="space-y-4 flex-1">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        {!isBlindMode && (
                          <button
                            onClick={() =>
                              navigate(`/chat/${app.studentEmail}`)
                            }
                            className="text-[10px] bg-paper hover:bg-paper text-ink-dark font-bold border border-border px-2.5 py-0.5 rounded-full transition shadow-sm"
                          >
                            {t('applicantsHub.chat')}
                          </button>
                        )}
                      </div>
                      <p className="text-xs text-ink-400 font-medium">
                        {isBlindMode
                          ? t('applicantsHub.mockEmail')
                          : app.studentEmail}
                      </p>
                    ...
                    <div>
                      <span className="text-[10px] uppercase font-bold text-ink-400 block mb-1">
                        {t('applicantsHub.declaredSkills')}
                      </span>
                    ...
                    {app.aiRationale && (
                      <div className="bg-paper border border-border p-3 rounded-xl text-left max-w-lg mt-2">
                        <p className="text-[9px] font-extrabold text-ink-dark flex items-center gap-1 uppercase tracking-wider">
                          <Bot className="w-3 h-3" />{' '}
                          {t('applicantsHub.aiMatchingInsights')}
                        </p>
                  ...
                  <div className="flex flex-row lg:flex-col justify-between lg:justify-center items-center gap-2 border-t lg:border-t-0 lg:border-l lg:border-r border-ink-100 pt-4 lg:pt-0 px-0 lg:px-8 min-w-[120px]">
                    <div className="text-left lg:text-center">
                      <span className="text-[9px] uppercase font-bold text-ink-400 block">
                        {t('applicantsHub.matchScore')}
                      </span>
                      <span
                  ...
                  <div className="flex flex-col justify-between items-stretch lg:items-end gap-4 border-t lg:border-t-0 border-ink-100 pt-4 lg:pt-0 min-w-[200px]">
                    <div className="text-left lg:text-right space-y-1.5">
                      ...
                      {app.resumeUrl ? (
                        ...
                        <span className="text-xs text-ink-400 italic flex items-center gap-1">
                          <FileText className="w-3 h-3 opacity-50" />{' '}
                          {t('applicantsHub.noResume')}
                        </span>
                      )}
                      ...
                      <div className="flex items-center lg:justify-end gap-3 mt-1.5 text-[10px]">
                        ...
                        {app.portfolioUrl && (
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="text-green-600 hover:text-ink-dark font-semibold flex items-center gap-1"
                            title={t('applicantsHub.portfolio')}
                          >
                            <Globe className="w-3 h-3" />{' '}
                            <span className="hover:underline">
                              {t('applicantsHub.portfolio')}
                            </span>
                          </a>
                        )}
                      </div>
                    ...
                    {app.extensionRequests &&
                      ...
                      app.extensionRequests[app.extensionRequests.length - 1]
                        .status === 'Pending' && (
                        <div className="mt-3 bg-marigold-50 border border-marigold-100 p-3.5 rounded-xl text-xs text-ink-800 space-y-2 animate-fade-in w-full text-left">
                          <div>
                            <span className="font-extrabold text-marigold-700 flex items-center gap-1 mb-0.5">
                              <Clock className="w-3 h-3" />{' '}
                              {t('applicantsHub.extensionRequested')}
                            </span>
                            <p className="text-[11px] text-ink-600 leading-normal">
                              {t('applicantsHub.wants')}{' '}
                              <strong>
                                {
                                  app.extensionRequests[
                                ...
                                {t('applicantsHub.moreDays')}
                              </strong>
                              . {t('applicantsHub.reason')}:"
                              {
                                app.extensionRequests[
                                  app.extensionRequests.length - 1
                                ].reason
                    ...
                    <div className="flex gap-2 mt-2 w-full">
                      {app.status === 'Pending' && (
                        <>
                          ...
                          <button
                            ...
                            className="flex-1 lg:flex-initial px-4 py-2 font-bold text-xs rounded-xl transition shadow-sm"
                          >
                            {t('applicantsHub.accept')}
                          </button>
                        </>
                      )}
                        ...
                        <div className="w-full text-left">
                          ...
                          {app.extendedDeadline && (
                            <p className="text-[10px] text-marigold-500 mt-1 font-bold flex items-center gap-1">
                              ...
                              {t('applicantsHub.extendedDeadline')}{' '}
                              {new Date(
                                app.extendedDeadline
                              ).toLocaleDateString()}
                            </p>
                      ...
                      {app.status === 'Flagged' && (
                        ...
                        >
                          <AlertTriangle className="w-3 h-3" />{' '}
                          {t('applicantsHub.plagiarismAlert', {
                            score: app.plagiarismScore || 0,
                          })}
                        </button>
                      )}
                      ...
                      {app.status === 'Completed' && (
                        <div className="text-left lg:text-right flex flex-col items-start lg:items-end">
                          ...
                          {app.feedbackText && (
                            <p
                              className="text-[10px] text-ink-400 italic max-w-[200px] truncate"
          ...
          <div className="wm-panel rounded-xl max-w-lg w-full shadow-sm p-6 animate-fade-in">
            <div className="flex justify-between items-center border-b pb-3 mb-4">
              <h3 className="font-bold text-base text-ink-900">
                {t('applicantsHub.verifySolutionTitle')}{' '}
                {activeAppToReview.studentName}
              </h3>
              <button
                onClick={() => {
            ...
            {activeAppToReview.submissionVersions &&
              activeAppToReview.submissionVersions.length > 1 && (
                <div className="mb-3 flex items-center justify-between bg-marigold-50/50 border border-marigold-100 p-2.5 rounded-xl text-xs">
                  <span className="text-[10px] font-black text-marigold-700 uppercase tracking-wider">
                    {t('applicantsHub.solutionIterations')}
                  </span>
                  <select
            ...
            {(() => {
              const hasVersions =
                activeAppToReview.submissionVersions &&
                activeAppToReview.submissionVersions.length > 0;
              const currentVer =
                hasVersions &&
                selectedVerIdx >= 0 &&
                selectedVerIdx < activeAppToReview.submissionVersions.length
                ...
                <div className="bg-ink-50 border border-ink-100 p-4 rounded-xl mb-4 space-y-3 text-xs">
                  <p className="text-ink-700">
                    <strong className="text-ink-900">
                      {t('applicantsHub.projectGigs')}
                    </strong>{' '}
                    {activeAppToReview.projectTitle}
                  </p>
                  <div className="flex justify-between items-center gap-2">
                    <p className="text-ink-700 truncate flex-1">
                      <strong className="text-ink-900">
                        {t('applicantsHub.submittedUrl')}
                      </strong>
                      {''}
                      <a
                        href={currentVer.submissionLink}
                        target="_blank"
                        rel="noreferrer"
                        className="text-marigold-500 hover:underline break-all font-bold"
                      >
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        setActiveFile(
                          Object.keys(files)[0] || 'submission.txt'
                        );
                        setShowSandbox(true);
                      }}
                    ...
                    >
                      ...
                      {t('applicantsHub.auditSandbox')}
                    </button>
                  </div>
                  {currentVer.githubRepoUrl && (
                    <p className="text-ink-700">
                      <strong className="text-ink-900">
                        {t('applicantsHub.githubRepo')}
                      </strong>
                      {''}
                      <a
                        href={currentVer.githubRepoUrl}
                        target="_blank"
                        rel="noreferrer"
                        className="text-ink-dark hover:underline break-all font-bold"
                      >
                        {currentVer.githubRepoUrl} ↗
                      </a>
                    </p>
                  ...
                  {currentVer.liveDeploymentUrl && (
                    <p className="text-ink-700">
                      <strong className="text-ink-900">
                        {t('applicantsHub.livePreview')}
                      </strong>
                      {''}
                      <a
                        href={currentVer.liveDeploymentUrl}
                        target="_blank"
                        rel="noreferrer"
                        className="text-emerald-600 hover:underline break-all font-bold"
                      >
                        {currentVer.liveDeploymentUrl} ↗
                      </a>
                    </p>
                  ...
                  <p className="text-ink-700 leading-normal">
                    <strong className="text-ink-900">
                      {t('applicantsHub.solutionDescription')}
                    </strong>{' '}
                    {currentVer.submissionText}
                  </p>
                  ...
                  {currentVer.selfAssessment && (
                    <div className="bg-white border rounded-xl p-3 space-y-1.5">
                      <span className="text-[9px] font-bold text-ink-400 uppercase tracking-wider block">
                        {t('applicantsHub.studentRubric')}
                      </span>
                      <div className="grid grid-cols-3 gap-2 text-center text-[11px]">
                        <div className="bg-ink-50 p-1.5 rounded-lg">
                          <p className="text-ink-400 font-bold">
                            {t('applicantsHub.codeQuality')}
                          </p>
                          <p className="font-extrabold text-marigold-700 mt-0.5">
                            {currentVer.selfAssessment.codeQuality || 'N/A'}/10
                          </p>
                        </div>
                        <div className="bg-ink-50 p-1.5 rounded-lg">
                          <p className="text-ink-400 font-bold">
                            {t('applicantsHub.correctness')}
                          </p>
                          <p className="font-extrabold text-marigold-700 mt-0.5">
                            {currentVer.selfAssessment.correctness || 'N/A'}/10
                          </p>
                        </div>
                        <div className="bg-ink-50 p-1.5 rounded-lg">
                          <p className="text-ink-400 font-bold">
                            {t('applicantsHub.docs')}
                          </p>
                          <p className="font-extrabold text-marigold-700 mt-0.5">
                            {currentVer.selfAssessment.documentation || 'N/A'}
                            /10
                          </p>
                        </div>
                      </div>
                    ...
                    >
                      <p className="font-bold flex items-center gap-1.5">
                        {currentVer.aiDeclaration.usedAi ? (
                          <>
                            <Bot className="w-3 h-3" />{' '}
                            {t('applicantsHub.aiCoauthored', {
                              percentage: currentVer.aiDeclaration.aiPercentage,
                              tools: currentVer.aiDeclaration.toolsUsed,
                            })}
                          </>
                        ) : (
                          <>
                  ...
                  {activeAppToReview.plagiarismScore > 0 && (
                    ...
                    >
                      <p className="flex items-start gap-1.5">
                        ...
                        <span>
                          {t('applicantsHub.codeSimilarityAudit', {
                            score: activeAppToReview.plagiarismScore,
                          })}
                          {activeAppToReview.plagiarismScore > 20 &&
                            t('applicantsHub.highMatchWarning')}
                        </span>
                      </p>
                    </div>
                  )}
                  ...
                  {activeAppToReview.lintWarnings &&
                    activeAppToReview.lintWarnings.length > 0 && (
                      <div className="bg-amber-50/50 border border-amber-150 text-amber-900 p-2.5 rounded-xl text-[11px] text-left">
                        <p className="font-extrabold mb-1 flex items-center gap-1.5">
                          ...
                          {t('applicantsHub.syntaxLintAdvisories')}
                        </p>
                  ...
                  {activeAppToReview.matchScore !== null &&
                    activeAppToReview.matchScore !== undefined && (
                      <div className="bg-marigold-50 border border-marigold-100 p-2.5 rounded-xl text-[11px] text-marigold-950 text-left">
                        <p className="font-extrabold flex items-center gap-1.5">
                          <Bot className="w-3 h-3" />{' '}
                          {t('applicantsHub.aiAuditorReport')}
                        </p>
                        <p className="mt-0.5">
                          {t('applicantsHub.grade')}:{' '}
                          <strong>{activeAppToReview.matchScore}/100</strong>.{' '}
                          {t('applicantsHub.rationale')}:"
                          {activeAppToReview.aiRationale ||
                            t('applicantsHub.noDescription')}
                          "
                        </p>
                      </div>
                    )}
                  ...
                  {currentVer.submissionLink &&
                    (currentVer.submissionLink.includes('figma.com') ||
                      currentVer.submissionLink.includes('figma.fun')) && (
                      <div className="border rounded-xl overflow-hidden mt-3 shadow-sm bg-white">
                        <p className="text-[9px] uppercase font-bold text-ink-400 bg-ink-100 px-3 py-1.5 border-b select-none flex items-center gap-1.5">
                          <FileCheck className="w-3 h-3" />{' '}
                          {t('applicantsHub.figmaLivePreview')}
                        </p>
                        <iframe
                  ...
                  <p className="text-ink-400 text-[10px] pt-1">
                    {t('applicantsHub.submittedOn')}{' '}
                    {new Date(
                      currentVer.submittedAt || activeAppToReview.submittedAt
                    ).toLocaleString()}
                  </p>
                </div>
              );
            ...
            <form onSubmit={handleCompleteReview} className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-ink-500 uppercase mb-1">
                  {t('applicantsHub.verifyFeedbackTitle')}
                </label>
                <textarea
              ...
              <div className="border-t pt-3 space-y-3">
                <div>
                  <label className="block text-[10px] font-extrabold text-ink-dark uppercase tracking-wider mb-1">
                    {t('applicantsHub.candidateRating')}
                  </label>
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-[10px] text-ink-400 font-extrabold ml-2">
                      ({rating} / 5 {t('applicantsHub.stars')})
                    </span>
                  </div>
                </div>
                ...
                <div>
                  <label className="block text-[10px] font-extrabold text-ink-dark uppercase tracking-wider mb-1">
                    {t('applicantsHub.performanceReviewTitle')}
                  </label>
                  <textarea
                    placeholder={t(
                      'applicantsHub.performanceReviewPlaceholder'
                    )}
                    value={ratingReview}
                    onChange={(e) => setRatingReview(e.target.value)}
                    rows={2}
                    className="w-full bg-ink-50 border border-ink-200 text-xs px-3.5 py-2 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#F5A623] resize-none"
                  />
                </div>
              </div>
              <div className="flex justify-between items-center w-full pt-2">
                <div className="flex gap-2">
                  ...
                  >
                    {t('applicantsHub.requestRevision')}
                  </button>
                  <button
                    type="button"
                ...
                <div className="flex gap-2">
                  ...
                  >
                    {submittingReview
                      ? t('applicantsHub.processing')
                      : t('applicantsHub.verifyAndComplete')}
                  </button>
                </div>
              </div>
            </form>
        ...
        <div className="fixed inset-0 bg-ink-900/60 z-50 flex items-center justify-end p-0 animate-fade-in select-none">
          <div className="bg-ink-955 text-ink-100 w-full max-w-4xl h-full flex flex-col shadow-sm border-l border-ink-800">
            ...
            <div className="bg-ink-900 border-b border-ink-800 px-6 py-4 flex justify-between items-center">
              <div>
                <h3 className="text-sm font-black uppercase text-marigold-400">
                  {t('applicantsHub.sandboxTitle')}
                </h3>
                <p className="text-[11px] text-ink-400 mt-0.5">
                  {t('applicantsHub.auditing')}:{' '}
                  {activeAppToReview.projectTitle} ({t('applicantsHub.by')}{' '}
                  {activeAppToReview.studentName})
                </p>
              </div>
              <button
            ...
            <div className="flex-1 flex overflow-hidden">
              ...
              <div className="w-56 bg-ink-900/50 border-r border-ink-800 p-4 space-y-4 overflow-y-auto">
                <div>
                  <span className="text-[9px] font-black text-ink-500 uppercase tracking-widest block mb-2">
                    {t('applicantsHub.workspaceFiles')}
                  </span>
                  <div className="space-y-1.5 text-xs">
                    {Object.keys(getSubmissionFiles(activeAppToReview)).map(
                      (filename) => (
                        <button
                          key={filename}
                ...
                <div className="border-t border-ink-800 pt-4">
                  <span className="text-[9px] font-black text-ink-500 uppercase tracking-widest block mb-2">
                    {t('applicantsHub.checklistAudit')}
                  </span>
                  <div className="space-y-2 text-[10px] text-ink-400 font-semibold">
                    <p className="flex items-center gap-1.5 text-emerald-500 font-bold">
                      <CheckCircle className="w-3 h-3" />{' '}
                      {t('applicantsHub.securityClean')}
                    </p>
                    <p className="flex items-center gap-1.5 text-emerald-500 font-bold">
                      <CheckCircle className="w-3 h-3" />{' '}
                      {t('applicantsHub.lintStandardOk')}
                    </p>
                    <p className="flex items-center gap-1.5 text-emerald-500 font-bold">
                      <CheckCircle className="w-3 h-3" />{' '}
                      {t('applicantsHub.dependenciesVerified')}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex-1 flex flex-col bg-ink-950 font-mono text-xs select-text">
                ...
                <pre className="flex-1 p-6 overflow-y-auto leading-relaxed text-ink-300 bg-ink-950/50 whitespace-pre-wrap">
                  <code>
                    {getSubmissionFiles(activeAppToReview)[activeFile] ||
                      `// ${t('applicantsHub.fileNotFound')}`}
                  </code>
                </pre>
              </div>
            </div>
            ...
            <div className="bg-ink-900 border-t border-ink-800 px-6 py-4 flex justify-between items-center gap-4">
              <span className="text-[10px] text-ink-400 font-bold uppercase tracking-wider flex items-center gap-1.5">
                {t('applicantsHub.escrowActive')} (
                <ShieldCheck className="w-3 h-3" /> {t('applicantsHub.locked')})
              </span>
              <div className="flex gap-2">
                <button
                  onClick={() => {
                ...
                >
                  {t('applicantsHub.flagDisputeButton')}
                </button>
                <button
                  onClick={() => {
                    setShowSandbox(false);
                ...
                >
                  {t('applicantsHub.approveSolution')}
                </button>
              </div>
            </div>
          </div>
