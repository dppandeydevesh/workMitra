...
export default function AdminDashboard() {const toast = useToast();
...
"INF [Plagiarism] Bigram similarity module initialized"]);
 ...
 const handleSaveConfig = (e) => {e.preventDefault();
 setSavingConfig(true);
 setTimeout(() => {setSavingConfig(false);
...
`INF [Config] Minimum deposit threshold set to ₹${minDeposit}`,
 ...
 const handleBackupDatabase = () => {setBackingUp(true);
 setBackupStatus(t("admin.backup.status.initializing"));
 setTimeout(() => {setBackupStatus(t("admin.backup.status.compressing"));
 setTimeout(() => {setBackingUp(false);
...
`INF [System] Automated backup archive ${filename} created`,
 ...
 const handleResolveDispute = async (applicationId, decision) => {if (!window.confirm(t("admin.confirm.resolve_dispute", { decision: decision ==="release" ? t("admin.action.release_payout") : t("admin.action.refund_recruiter")}))) return;
 
 setResolvingId(applicationId);
 try {const res = await fetch(`${API_BASE_URL}/api/admin/disputes/${applicationId}/resolve`, { credentials:"include",
 method:"POST",
 headers: {
"Content-Type":"application/json",
 ...
 if (res.ok) {toast.success(t("admin.toast.dispute_resolved", { status: decision ==="release" ? t("admin.toast.payout_released") : t("admin.toast.budget_refunded")}));
 fetchAdminData();
 ...
 const handleVerifyCompany = async (companyEmail) => {if (!window.confirm(t("admin.confirm.verify_company", { companyEmail}))) return;
 
 setVerifyingEmail(companyEmail);
 try {const res = await fetch(`${API_BASE_URL}/api/admin/companies/${companyEmail}/verify`, { credentials:"include",
 method:"POST",
 headers: {}
});
 const data = await res.json();
 if (res.ok) {toast.success(t("admin.toast.company_verified"));
 fetchAdminData();
 ...
 {[...Array(6)].map((_, i) => (
 <div key={i} className="glass-card p-6 rounded-xl skeleton-loader h-32"></div>
 ))}
 </div>
 ...
 <p className="text-sm font-medium text-ink-500 mt-2">{t("admin.description")}</p>
 </div>
 </div>
 <button
 onClick={fetchAdminData}
 ...
 <span className="text-3xl font-black text-ink-800 drop-shadow-sm relative z-10">₹{metrics.disputedEscrow.toLocaleString()}</span>
 </motion.div>
 </motion.div>
 )}
 ...
 <p className="text-sm font-medium text-ink-500 mt-1">{t("admin.disputes.description")}</p>
 </div>
 </div>
 ...
 <p className="text-sm font-medium text-ink-500 italic">{t("admin.disputes.empty")}</p>
 </div>
 ) : (
 ...
 <tr key={dispute._id} className={`transition-colors duration-200 hover:bg-white/40 ${idx % 2 === 0 ?'bg-transparent' :'bg-ink-50/30'}`}>
 <td className="py-5 px-6">
 <span className="font-bold text-ink-900 block text-base">{dispute.projectId?.title || t("admin.disputes.fallback.project_title")}</span>
 ...
 <td className="py-5 px-6 font-black text-emerald-600 text-base">₹{dispute.projectId?.budget?.toLocaleString()}</td>
 <td className="py-5 px-6">
 <div className="max-w-xs text-sm italic border-l-2 border-marigold-200 pl-3 py-1">
"{dispute.feedbackText || t("admin.disputes.fallback.no_feedback")}"</div>
 </td>
 ...
 <p className="text-sm font-medium text-ink-500 mt-1">{t("admin.companies.description")}</p>
 </div>
 </div>
 ...
 <div className="py-12 flex flex-col items-center justify-center text-center relative z-10">
 <Inbox className="w-16 h-16 text-ink-300 mb-4 opacity-50" />
 <p className="text-sm font-medium text-ink-500 italic">{t("admin.companies.empty")}</p>
 </div>
 ) : (
 <div className="overflow-x-auto relative z-10 rounded-xl border border-ink-200/30 shadow-sm bg-white/20">
 <table className="w-full text-left text-sm border-collapse">
 ...
 <tr key={company._id} className={`transition-colors duration-200 hover:bg-white/40 ${idx % 2 === 0 ?'bg-transparent' :'bg-ink-50/30'}`}>
 <td className="py-5 px-6 font-bold text-ink-900 text-base">
 {company.companyName || company.fullName}
 </td>
 <td className="py-5 px-6 font-medium text-ink-600">{company.email}</td>
 <td className="py-5 px-6 font-medium text-ink-500">{company.mobile || <span className="italic opacity-60">{t("admin.companies.fallback.na")}</span>}</td>
 <td className="py-5 px-6">
 {company.isVerified ? (
 ...
 <span className="text-xs text-ink-400 font-medium italic bg-ink-100/50 px-3 py-1.5 rounded-lg border border-ink-200/50">{t("admin.companies.no_action")}</span>
 )}
 </td>
 </tr>
 ))}
 ...
 <p className="text-sm font-medium text-ink-500 mt-1">{t("admin.config.description")}</p>
 </div>
 </div>
 ...
 <input
 type="range"min="0"max="50"value={commissionRate}
 ...
 <div className="bg-white/30 p-5 rounded-xl border border-white/40 shadow-sm transition-colors focus-within:border-marigold-500/50">
 <label className="block font-bold text-ink-700 mb-3 text-sm uppercase tracking-wide">{t("admin.config.min_deposit")}</label>
 <div className="relative">
 <span className="absolute left-4 top-1/2 -translate-y-1/2 text-ink-500 font-bold">₹</span>
 <input
 type="number"min="500"max="100000"step="500"value={minDeposit}
 ...
 <p className="text-sm font-medium text-ink-500 mt-1">{t("admin.backup.description")}</p>
 </div>
 </div>
 <button
